...
const Packing_Material_Issue = (props) => {
  ...
  const fetchProcessDelayData = (batchNo) => {
    setLoading(true);
    axios
      .get(`${ API.prodUrl}/Precot/api/cottonBall/03.GetPackingMeterial`, {
    ...
    if (fromDate && toDate) {
      ...
      axios
        .get(
          `${
          API.prodUrl
          }/Precot/api/cottonBall/03.GetPackingMeterialPde?batch_no=${order_no}&fromdate=${moment(
            fromDate
  ...
  const handleSubmit = () => {
    if (props.batchNo) {
      ...
      axios
        .post(
          `${ API.prodUrl}/Precot/api/cottonBall/03.SubmitPackingMeterialIssue`,
          payload,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
