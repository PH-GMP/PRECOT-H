...
//		String findLastNote();
		...
		@Query(value = "SELECT SUM(CASE WHEN DEVELOPMENTSUPERVISOR_STATUS = 'SUPERVISOR_SAVED'THEN 1 ELSE 0 END) AS devSupervisorCount ,\r\n"
				+ "SUM(CASE WHEN QC_STATUS = 'WAITING_FOR_APPROVAL' THEN 1 ELSE 0 END) AS qcCount,\r\n"
				+ "SUM(CASE WHEN QC_STATUS = 'QC_APPROVED' AND QA_STATUS = 'WAITING_FOR_APPROVAL' THEN 1 ELSE 0 END) AS qaCount,\r\n"
				+ "SUM(CASE WHEN PPC__STATUS IS NULL AND (QC_STATUS = 'QC_APPROVED' AND QA_STATUS = 'QA_APPROVED') THEN 1 ELSE 0 END) AS ppcCount,\r\n"
				+ "SUM(CASE WHEN BLEACHING_STATUS IS NULL AND (QC_STATUS = 'QC_APPROVED' AND QA_STATUS = 'QA_APPROVED') THEN 1 ELSE 0 END) AS bleachingCount,\r\n"
				+ "SUM(CASE WHEN SPUNLACE_STATUS IS NULL AND (QC_STATUS = 'QC_APPROVED' AND QA_STATUS = 'QA_APPROVED') THEN 1 ELSE 0 END) AS spunlaceCount,\r\n"
				+ "SUM(CASE WHEN PAD_PUNCHING_STATUS IS NULL AND (QC_STATUS = 'QC_APPROVED' AND QA_STATUS = 'QA_APPROVED') THEN 1 ELSE 0 END) AS padPunchingCount,\r\n"
				+ "SUM(CASE WHEN DRY_GOODS_STATUS IS NULL AND (QC_STATUS = 'QC_APPROVED' AND QA_STATUS = 'QA_APPROVED') THEN 1 ELSE 0 END) AS DryGoodsCount\r\n"
				+ "FROM precot.PRODUCT_DEVELOPMENT_SHEET_F001", nativeQuery = true)
		List<Object[]> getProductDevelopmentSheetStatusCounts();
