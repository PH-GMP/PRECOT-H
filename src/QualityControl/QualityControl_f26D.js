...
const QualityControl_f26D = () => {
  ...
  const handleBack = () => {
    navigate("/Precot/QualityControl/F-026D/Summary");
  };
  const rejectFlow = () => {
    setRejectModal(true);
  };
      ...
      const fetchData = async () => {
        try {
          const response = await axios.get(
            `${   API.prodUrl}/Precot/api/QcForm/getCotttonWoolPleatF26D?product=${productName}&customer=${customerName}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
  ...
  const runValidations = () => {
    ...
    if (
      !validateField(
        ...
        lowerBoundGross,
        upperBoundGross,
        "Gross weight with bag (g) actual should be within " +
          lowerBoundGross +
          " to " +
          upperBoundGross
      )
    ...
    const flodsStandard = Number(formData.no_of_folds_standard);
    const lowerBoundFolds = flodsStandard - 2;
    const upperBoundFolds = flodsStandard + 2;

    if (
      !validateField(
        formData.no_of_folds_actual,
        lowerBoundFolds,
        upperBoundFolds,
        "No. of Folds actual should be within " +
          lowerBoundFolds +
          " to " +
          upperBoundFolds
      )
  ...
  const handleSave = async () => {
    ...
    if (role == "ROLE_CHEMIST") {
      ...
      apiurl = `${   API.prodUrl}/Precot/api/QcForm/SaveCottonWoolPleatF26D`;
      payload = {
        formatNo: "PH-QCL01/F-026D",
        revisionNo: "01",
        formatName: "CERTIFICATE OF ANALYSIS FOR COTTON PLEAT",
        ...
        no_of_folds_standard: formData.no_of_folds_standard,
        no_of_folds_actual: formData.no_of_folds_actual,
        no_of_folds_remarks: formData.no_of_folds_remarks || "NA",
      ...
      apiurl = `${   API.prodUrl}/Precot/api/QcForm/ApproveOrRejectF26D`;
  ...
  const handleSubmit = async () => {
    ...
    if (role == "ROLE_CHEMIST") {
      ...
      const getName = (key) => {
        switch (key) {
          ...
          case "gross_weight_actual":
            return "Gross weight with bag (g)";
          case "no_of_folds_standard":
          case "no_of_folds_actual":
            return "No. of Folds";
      ...
      apiurl = `${   API.prodUrl}/Precot/api/QcForm/SubmitCottonWoolPleatF26D`;
      payload = {
        formatNo: "PH-QCL01/F-026D",
        revisionNo: "01",
        formatName: "CERTIFICATE OF ANALYSIS FOR COTTON PLEAT",
  ...
  const handleBlur = (event) => {
    ...
    // --------- Test Type 1 ---------------
    const widthStandard = Number(formData.width_standard);
    const tenPercentWeight = (widthStandard / 100) * 10;
    const twentyFivePercent = (widthStandard / 100) * 25;
    const lowerBoundWidth = widthStandard - tenPercentWeight;
    const upperBoundWidth = widthStandard + twentyFivePercent;
    ...
    const flodsStandard = Number(formData.no_of_folds_standard);
    const tenPercentflods = (flodsStandard / 100) * 10;
    const twentyFivePercentflods = (flodsStandard / 100) * 25;
    const lowerBoundFolds = flodsStandard - tenPercentflods;
    const upperBoundFolds = flodsStandard + twentyFivePercentflods;

    if (
      (Number(formData.no_of_folds_actual) < lowerBoundFolds ||
        Number(formData.no_of_folds_actual) > upperBoundFolds) &&
      formData.no_of_folds_actual !== "" &&
      formData.no_of_folds_standard !== ""
    ) {
      validation.add(
        "No. of Folds actual should be within " +
          lowerBoundFolds +
      ...
      handleClear("no_of_folds_actual");
    }
  ...
  return (
    <>
      <BleachingHeader
        formName={"CERTIFICATE OF ANALYSIS FOR COTTON PLEAT"}
        formatNo={"PH-QCL01/F-026D"}
        unit={"UNIT H"}
        MenuBtn={
          <Button
          ...
          <div style={{ height: "50vh" }}>
            <table style={{ height: "100%" }}>
              ...
              <tr>
                ...
                <td style={{ textAlign: "center" }}>No. of Folds</td>
                <td style={{ textAlign: "center" }}>
                  {" "}
                  <Input
                    type="number"
                    ...
                    value={formData.no_of_folds_standard}
                    onChange={(e) => {
                      handleInput(e.target.value, "no_of_folds_standard");
                    }}
                    onKeyDown={(e) => {
                      handleDigit(e, 100, "need");
                    }}
                ...
                <td style={{ textAlign: "center" }}>
                  <Input
                    ...
                    value={formData.no_of_folds_actual}
                    onChange={(e) => {
                      handleInput(e.target.value, "no_of_folds_actual");
                    }}
                    onKeyDown={(e) => {
                      handleDigit(e, 100, "need");
                    }}
                    onBlur={handleBlur}
                    readOnly={
                      status.fieldStatus || formData.no_of_folds_standard == ""
                    }
                  ></Input>
                </td>
                <td style={{ textAlign: "center", padding: "0px" }}>
                  <Select
                    options={Lov1}
                    value={formData.no_of_folds_remarks}
                    style={{ textAlign: "center", width: "300px" }}
                    onInputKeyDown={(e) => {
                      handleSelectText(e, "no_of_folds_remarks");
                    }}
                    onChange={(e) => {
                      handleInput(e, "no_of_folds_remarks");
                    }}
                    dropdownStyle={{ textAlign: "center" }}
                    onBlur={handleBlur}
                    showSearch
...
export default QualityControl_f26D;
