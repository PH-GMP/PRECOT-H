...
const Equipment_Used_Process = (props) => {
  ...
  const [equipmentData, setEquipmentData] = useState([
    ...
    {
      equipmentName: "Wool Roll",
      equipmentCode: "PH-E/I-WRL/PLT16",
      calibrationDate: "",
      dueDate: "",
      checked_by: "",
      remarks: "",
  ...
  useEffect(() => {
    if (props.batchNo) {
      axios
        .get(
          `${ API.prodUrl}/Precot/api/CottonWoolRall/05.GetProcessingEquipments?batch_no=${props.batchNo}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await axios.post(
        `${ API.prodUrl}/Precot/api/CottonWoolRall/05.SaveProcessingEquipments`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${ API.prodUrl}/Precot/api/CottonWoolRall/05.SubmitProcessingEquipments`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
