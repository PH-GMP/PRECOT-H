	...
	public ResponseEntity<?> getHeaderDetails(String order_no) {
		...
		try {
			List<Object[]> orderResponse = baleconsumptionreportdrygoodsf001repository
					.fetchheaderdetailsForF006(order_no);
...
//	            map.put("grams", record[3]);
				...
				map.put("bagByBox", record[2]);
				map.put("grams", record[3]);
				map.put("ProdDesc", record[4]);
				map.put("Product", record[5]);
				map.put("POrder", record[6]);
	...
	public ResponseEntity<?> getFleecetDetails(String date, String shift, String order_no) {
		...
		try {
			List<Object[]> orderResponse = baleconsumptionreportdrygoodsf001repository.fetchfleecetReceiptForF006(date,
					shift, order_no);
...
//	            Map<String, Object> map = new HashMap<>();
	...
	public ResponseEntity<?> fetchStoppageDetailsF006(String date, String shift, String order_no, String machine_name) {
		...
		try {
			List<Object[]> results = baleconsumptionreportdrygoodsf001repository.fetchStoppagedetailsForF006(date,
					shift, order_no, machine_name);
	...
	public ResponseEntity<?> submitBaleConsumptionReportF001(BaleConsumptionReportDryGoodsF001 baleConsumptionReport,
		...
		try {
			...
			if (baleConsumptionReport.getFormatNo() == null)
				value = "formatNo";
			if (baleConsumptionReport.getSopNumber() == null)
				value = "SopNumber";
			if (baleConsumptionReport.getRevisionNo() == null)
				value = "revisionNo";
			if (baleConsumptionReport.getFormatName() == null)
				value = "formatName";
			if (baleConsumptionReport.getUnit() == null)
				value = "Unit";
			if (baleConsumptionReport.getShift() == null)
				value = "Shift";
			if (baleConsumptionReport.getDate() == null)
				value = "Date";
			if (baleConsumptionReport.getLaydown_no() == null)
				value = "LaydownNo";
			...
			if (id != null) {
				report = baleconsumptionreportdrygoodsf001repository.findById(id)
						.orElse(new BaleConsumptionReportDryGoodsF001());

				String[] ignoreProps = { "bale_report_id", "createdBy", "createdAt", "operator_submitted_by",
						"operator_submitted_on", "operator_submitted_id", "operator_sign", "hod_status", "hod_save_on",
						"hod_save_by", "hod_save_id", "hod_submit_on", "hod_submit_by", "hod_submit_id", "hod_sign",
						"hod_mail_status", "operator_signature_image", "hod_signature_image" };
				...
				if (!report.getOperator_status().equals(AppConstants.operatorApprove)
					...
					if (userRole.equalsIgnoreCase(AppConstantDryGoods.operatorRole)) {
						...
						int version = baleconsumptionreportdrygoodshistoryf001repository
								.getMaximumVersion(date1, shift, laydownNo).map(temp -> temp + 1).orElse(1);
						...
						return new ResponseEntity<>(
								new ApiResponse(false, "Unauthorized to submit Bale Consumption Report"),
			...
			return new ResponseEntity<>(new ApiResponse(false, "Failed to submit Bale Consumption Report: " + msg),
					HttpStatus.BAD_REQUEST);
		}
	...
	// APPROVE/REJECT
	public ResponseEntity<?> approveRejectBaleConsumptionReport(ApproveResponse approvalResponse,
			HttpServletRequest http) {
			...
			if (currentStatus.equalsIgnoreCase(AppConstantDryGoods.waitingstatus)) {
				if (userRole.equalsIgnoreCase("ROLE_HOD")
					...
					if (approvalResponse.getStatus().equalsIgnoreCase("Approve")) {
						...
						history = baleconsumptionreportdrygoodshistoryf001repository.fetchLastSubmittedRecord(date1,
								shift, laydownNo);
...
//***************************************************************************F003 PDE***************************************************************************************************************
	// -----------------------------------------------------------------------------------------------------------------------------------//
	// Save F003
	public ResponseEntity<?> saveDailyProductionCottonBalls(DailyProductionCottonBallsF003 cottonBallsDetails,
			HttpServletRequest http) {
		...
		try {
			...
			if (id != null) {
				...
				if (cottonBalls != null) {
					String[] ignoreProps = { "cottonballs_id", "createdBy", "createdAt", "operator_status",
							"operator_save_by", "operator_save_on", "operator_save_id", "operator_submitted_by",
							"operator_submitted_on", "operator_submitted_id", "operator_sign", "supervisor_status",
				...
				if (!userRole.equalsIgnoreCase(AppConstantDryGoods.operatorRole)) {
					return new ResponseEntity<>(
							new ApiResponse(false, userRole + " Unauthorized to save Cotton Balls Report"),
							HttpStatus.BAD_REQUEST);
				}
	...
	public ResponseEntity<?> submitDailyProductionCottonBallsF003(
			DailyProductionCottonBallsF003 dailyproductioncottonballs, HttpServletRequest http) {
		...
		try {
			...
			// Validate mandatory fields
			if (dailyproductioncottonballs.getFormatNo() == null)
				value = "formatNo";
			if (dailyproductioncottonballs.getSopNumber() == null)
				value = "SopNumber";
			if (dailyproductioncottonballs.getRevisionNo() == null)
				value = "revisionNo";
			if (dailyproductioncottonballs.getFormatName() == null)
				value = "formatName";
			if (dailyproductioncottonballs.getUnit() == null)
				value = "Unit";
			if (dailyproductioncottonballs.getShift() == null)
				value = "Shift";
			if (dailyproductioncottonballs.getDate() == null)
				value = "Date";
			...
			String[] ignoreProps = { "cottonballs_id", "createdBy", "createdAt", "operator_status", "operator_save_by",
					...
					"supervisor_submit_id", "supervisor_sign", "supervisior_mail_status", "hod_status", "hod_save_on",
					"hod_save_by", "hod_save_id", "hod_submit_on", "hod_submit_by", "hod_submit_id", "hod_sign",
					"hod_mail_status", "supervisor_signature_image", "hod_signature_image", "sliver_id" };

			if (id != null) {
				report = dailyproductioncottonballsf003repository.findById(id)
						.orElse(new DailyProductionCottonBallsF003());
				...
				if (!report.getOperator_status().equals(AppConstants.operatorApprove)
					...
					if (userRole.equalsIgnoreCase(AppConstantDryGoods.operatorRole)) {
						...
						BeanUtils.copyProperties(report, historyRecord, ignoreProps);
						...
						int version = dailyproductioncottonballshistoryf003repository
								.getMaximumVersion(date1, shift, machineName, order_no).map(temp -> temp + 1).orElse(1);
						historyRecord.setVersion(version);
						...
						for (SliverReceiptDetailsF003 line : lines) {
							...
							BeanUtils.copyProperties(line, lineHistory, ignoreProps);
...
//	                report.setOperator_signature_image(signature);
					...
					return new ResponseEntity<>(
							new ApiResponse(false, "Unauthorized to submit Daily ProductionT Report"),
							HttpStatus.FORBIDDEN);
				}
			}

		} catch (Exception ex) {
			return new ResponseEntity<>(new ApiResponse(false, "Unable to submit details! " + ex.getMessage()),
...
//		                    dailyproductionReport.setHod_signature_image(signature);
						...
						// Update history with the approval details
						history = dailyproductioncottonballshistoryf003repository.fetchLastSubmittedRecord(date1, shift,
								laydownNo, order_no);
...
//		                    baleConsumptionReport.setHod_signature_image(signature);
						...
						return new ResponseEntity<>(new ApiResponse(true, "Production Report Rejected Successfully"),
								HttpStatus.OK);
...
//		    						dailyproductionReport.setSupervisor_signature_image(signature);
		...
		return new ResponseEntity<>(new ApiResponse(true, "Supervisor Rejected Successfully"), HttpStatus.OK);
	...
	public ResponseEntity<?> summaryDailyProductionCottonF003() {
		...
		try {
			...
			if (userRole.equalsIgnoreCase(AppConstantsSplunance.hodRole)
					|| userRole.equalsIgnoreCase(AppConstantDryGoods.designeeRole)
					|| userRole.equalsIgnoreCase(AppConstantDryGoods.supervisiorRole)) {
	...
	public ResponseEntity<?> fetchStoppageDetailsF003(String date, String shift, String order_no, String machine_name) {
		...
		try {
			List<Object[]> results = dailyproductioncottonballsf003repository.fetchStoppageF003(date, shift, order_no,
					machine_name);
...
//--------------------------------------------------------------------------------------F011--------------------------------------------------------------------------------
	 ...
	 * saveBallpleateAndWoolRollFinishedGoodsTransferRecordF011(
	 * BallpleateAndWoolRollFinishedGoodsTransferRecordF011 transferRecordDetails,
	 * HttpServletRequest http) {
	 * 
	 * BallpleateAndWoolRollFinishedGoodsTransferRecordF011 transferRecord = new
	 * BallpleateAndWoolRollFinishedGoodsTransferRecordF011(); Long id =
	 * transferRecordDetails.getFinished_goods_id(); try { String value = ""; String
	 * createdBy = ""; Date createdAt = null;
	 * 
	 * String userRole = getUserRole(); Long userId = sca.getUserIdFromRequest(http,
	 * tokenProvider); String userName = userRepository.getUserName(userId);
	 * LocalDateTime currentDate = LocalDateTime.now(); Date date =
	 * Date.from(currentDate.atZone(ZoneId.systemDefault()).toInstant());
	 * 
	 * // Mandatory fields validation if (transferRecordDetails.getDate() == null)
	 * value = "Date"; if (transferRecordDetails.getShift() == null) value =
	 * "Shift"; if (transferRecordDetails.getPo_no() == null) value = "PO No"; if
	 * (transferRecordDetails.getProduct_name() == null) value = "Product Name";
	 * 
	 * if (!"".equals(value)) { return new ResponseEntity<>(new ApiResponse(false,
	 * "Should Fill Mandatory Fields! " + value), HttpStatus.BAD_REQUEST); }
	 * 
	 * if (id != null) { transferRecord =
	 * ballpleateandwoolrollfinishedgoodstransferrecordf011repository.findById(id).
	 * orElse(null);
	 * 
	 * if (transferRecord != null) { String[] ignoreProps = { "finished_goods_id",
	 ...
	 * 
	 * ballpleateandwoolrollfinishedgoodstransferrecordf011repository.save(
	 * transferRecord);
	 * 
	 * } else { return new ResponseEntity<>(new ApiResponse(false,
	 * "Unauthorized to save Finished Goods Transfer Record"),
	 * HttpStatus.FORBIDDEN); } }
	 * 
	 * } else { if (!userRole.equalsIgnoreCase(AppConstantDryGoods.supervisiorRole))
	 * { return new ResponseEntity<>(new ApiResponse(false, userRole +
	 * " Unauthorized to save Finished Goods Transfer Record"),
	 * HttpStatus.BAD_REQUEST); }
	 * 
	 * BeanUtils.copyProperties(transferRecordDetails, transferRecord); if
	 * (userRole.equalsIgnoreCase(AppConstantDryGoods.supervisiorRole)) {
	 * 
	 * transferRecord.setSupervisor_status(AppConstants.supervisorSave);
	 * transferRecord.setSupervisor_save_by(userName);
	 ...
	 * 
	 * ballpleateandwoolrollfinishedgoodstransferrecordf011repository.save(
	 * transferRecord); } } } catch (Exception ex) { String msg = ex.getMessage();
	 * logger.error("Unable to save Finished Goods Transfer Record: " + msg); return
	 * new ResponseEntity<>(new ApiResponse(false,
	 * "Failed to save Finished Goods Transfer Record: " + msg),
	 * HttpStatus.BAD_REQUEST); }
	 * 
	 * return new ResponseEntity<>(transferRecord, HttpStatus.CREATED); } //Submit
	 * F011 public ResponseEntity<?>
	 * submitBallpleateAndWoolRollFinishedGoodsTransferRecordF011(
	 * BallpleateAndWoolRollFinishedGoodsTransferRecordF011 transferRecordDetails,
	 * HttpServletRequest http) {
	 * 
	 * BallpleateAndWoolRollFinishedGoodsTransferRecordF011 transferRecord = new
	 * BallpleateAndWoolRollFinishedGoodsTransferRecordF011(); Long id =
	 * transferRecordDetails.getFinished_goods_id();
	 * 
	 * try { String value = ""; String userRole = getUserRole(); Long userId =
	 * sca.getUserIdFromRequest(http, tokenProvider); String userName =
	 * userRepository.getUserName(userId); LocalDateTime currentDate =
	 * LocalDateTime.now(); Date date =
	 * Date.from(currentDate.atZone(ZoneId.systemDefault()).toInstant());
	 * 
	 * // Validate mandatory fields if (transferRecordDetails.getDate() == null)
	 * value = "Date"; if (transferRecordDetails.getShift() == null) value =
	 * "Shift"; if (transferRecordDetails.getPo_no() == null) value = "PO No"; if
	 * (transferRecordDetails.getProduct_name() == null) value = "Product Name";
	 * 
	 ...
	 * BeanUtils.copyProperties(transferRecordDetails, transferRecord, ignoreProps);
	 * 
	 * if (!transferRecord.getSupervisor_status().equals(AppConstants.
	 * supervisorApprovedStatus) ||
	 * transferRecord.getSupervisor_status().equals(AppConstants.
	 * supervisorRejectedStatus)) {
	 * 
	 * if (userRole.equalsIgnoreCase(AppConstantDryGoods.supervisiorRole)) { //
	 * Update the status and submission details
	 ...
	 * 
	 * // Fetch and set the supervisor's signature Optional<UserImageDetails>
	 * imageDetailsOpt = imageRepository.fetchItemDetailsByUsername(userName);
	 * byte[] signature =
	 * imageDetailsOpt.map(UserImageDetails::getImage).orElse(null); //
	 ...
	 * 
	 * // Create audit history record
	 * BallpleateAndWoolRollFinishedGoodsTransferRecordHistoryF011 historyRecord =
	 * new BallpleateAndWoolRollFinishedGoodsTransferRecordHistoryF011();
	 * BeanUtils.copyProperties(transferRecord, historyRecord);
	 * 
	 * String date1 = historyRecord.getDate(); String shift =
	 * historyRecord.getShift();
	 * 
	 * int version =
	 * ballpleateandwoolrollfinishedgoodstransferrecordhistoryf011repository.
	 * getMaximumVersion(date1, shift) .map(temp -> temp + 1).orElse(1);
	 * historyRecord.setVersion(version);
	 * 
	 * ballpleateandwoolrollfinishedgoodstransferrecordhistoryf011repository.save(
	 * historyRecord);
	 * 
	 * // Send notification email try { //
	 * drygoodsmailfunction.sendEmailToHODF011(transferRecord); } catch (Exception
	 * ex) { return new ResponseEntity<>(new ApiResponse(false,
	 * "Submitted but unable to send email!"), HttpStatus.OK); }
	 * 
	 * } else { return new ResponseEntity<>(new ApiResponse(false,
	 * "Unauthorized to submit Finished Goods Transfer Record"),
	 * HttpStatus.FORBIDDEN); } } else { return new ResponseEntity<>(new
	 * ApiResponse(false, "Invalid status."), HttpStatus.BAD_REQUEST); } } else { if
	 * (!userRole.equals(AppConstantDryGoods.supervisiorRole)) { return new
	 * ResponseEntity<>(new ApiResponse(false, userRole + " cannot submit details"),
	 * HttpStatus.BAD_REQUEST); }
	 * 
	 * if (userRole.equalsIgnoreCase(AppConstantDryGoods.supervisiorRole)) {
	 * transferRecordDetails.setSupervisor_status(AppConstants.
	 * supervisorApprovedStatus);
	 ...
	 * 
	 * // Send notification email try { //
	 * drygoodsmailfunction.sendEmailToHODF011(transferRecordDetails); } catch
	 * (Exception ex) { return new ResponseEntity<>(new ApiResponse(false,
	 * "Submitted but unable to send email!"), HttpStatus.OK); } } } } catch
	 * (Exception ex) { String msg = ex.getMessage();
	 * logger.error("Unable to submit Finished Goods Transfer Record: " + msg);
	 * return new ResponseEntity<>(new ApiResponse(false,
	 * "Failed to submit Finished Goods Transfer Record: " + msg),
	 * HttpStatus.BAD_REQUEST); }
	 * 
			...
			if (id != null) {
				...
				if (transferRecord != null) {
					...
					if (userRole.equalsIgnoreCase(AppConstantDryGoods.supervisiorRole)) {
						...
						return new ResponseEntity<>(
								new ApiResponse(false, "Unauthorized to save Finished Goods Transfer Record"),
								HttpStatus.FORBIDDEN);
					}
				}
				...
				if (!userRole.equalsIgnoreCase(AppConstantDryGoods.supervisiorRole)) {
					return new ResponseEntity<>(
							new ApiResponse(false, userRole + " Unauthorized to save Finished Goods Transfer Record"),
							HttpStatus.BAD_REQUEST);
				}
	...
	public ResponseEntity<?> submitBallpleateAndWoolRollFinishedGoodsTransferRecordF011(
		...
		try {
			...
			if (id != null) {
				...
				String[] ignoreProps = { "finished_goods_id", "createdBy", "createdAt", "supervisor_status",
						"supervisor_save_by", "supervisor_save_on", "supervisor_save_id", "supervisor_submit_on",
						"supervisor_submit_by", "supervisor_submit_id", "supervisor_sign",
...
//			                    transferRecord.setSupervisor_signature_image(signature);
						...
						if (lines != null && !lines.isEmpty()) {
							...
							int version = ballpleateandwoolrollfinishedgoodstransferrecordhistoryf011repository
									.getMaximumVersion(date1, shift).map(temp -> temp + 1).orElse(1);
							historyRecord.setVersion(version);
...
//			                        drygoodsmailfunction.sendEmailToHODF011(transferRecord);
								...
								return new ResponseEntity<>(
										new ApiResponse(false, "Submitted but unable to send email!"), HttpStatus.OK);
							}

						}
					} else {
						return new ResponseEntity<>(
								new ApiResponse(false, "Unauthorized to submit Finished Goods Transfer Record"),
								HttpStatus.FORBIDDEN);
					}
				} else {
					return new ResponseEntity<>(new ApiResponse(false, "Invalid status."), HttpStatus.BAD_REQUEST);
...
//			                    drygoodsmailfunction.sendEmailToHODF011(transferRecordDetails);
			...
			logger.error("Unable to submit Finished Goods Transfer Record: " + msg);
			return new ResponseEntity<>(
					new ApiResponse(false, "Failed to submit Finished Goods Transfer Record: " + msg),
					HttpStatus.BAD_REQUEST);
		}
	...
	public ResponseEntity<?> summaryGoodsTranferRecordF011() {
		...
		try {
			...
			if (userRole.equalsIgnoreCase(AppConstantDryGoods.supervisiorRole)) {
				processSetupDetails = ballpleateandwoolrollfinishedgoodstransferrecordf011repository
						.supervisorSummary();
			} else {
				return new ResponseEntity<>(userRole + " " + "Not authorised to access the form",
						HttpStatus.BAD_REQUEST);
		...
		try {
			...
			if (id != null) {
				...
				if (productionRecord != null) {
					String[] ignoreProps = { "pleate_id", "operator_status", "operator_save_by", "operator_save_on",
							...
							"hod_submit_id", "hod_sign", "hod_mail_status", "supervisor_signature_image",
							"hod_signature_image" };
				...
				if (!userRole.equalsIgnoreCase(AppConstantDryGoods.operatorRole)) {
					return new ResponseEntity<>(
							new ApiResponse(false, userRole + " Unauthorized to save Production Record"),
							HttpStatus.BAD_REQUEST);
				}
	...
	public ResponseEntity<?> submitDailyProductionPleateAndWoolRollF006(
			DailyProductionDetailsPleateAndWoolRollF006 dailyProductionDetails, HttpServletRequest http) {
		...
		try {
			...
			// Validate mandatory fields
			if (dailyProductionDetails.getFormatNo() == null)
				value = "formatNo";
			if (dailyProductionDetails.getSopNumber() == null)
				value = "SopNumber";
			if (dailyProductionDetails.getRevisionNo() == null)
				value = "revisionNo";
			if (dailyProductionDetails.getFormatName() == null)
				value = "formatName";
			if (dailyProductionDetails.getUnit() == null)
				value = "Unit";
			if (dailyProductionDetails.getShift() == null)
				value = "Shift";
			if (dailyProductionDetails.getDate() == null)
				value = "Date";
			...
			if (id != null) {
				report = dailyproductiondetailspleateandwoolrollf006repository.findById(id)
						.orElse(new DailyProductionDetailsPleateAndWoolRollF006());

				String[] ignoreProps = { "pleate_id", "createdBy", "createdAt", "operator_status", "operator_save_by",
						"operator_save_on", "operator_save_id", "operator_submitted_by", "operator_submitted_on",
						"operator_submitted_id", "operator_sign", "supervisor_status", "supervisor_save_on",
						"supervisor_save_by", "supervisor_save_id", "supervisor_submit_on", "supervisor_submit_by",
						"supervisor_submit_id", "supervisor_sign", "supervisior_mail_status", "hod_status",
				...
				if (!report.getOperator_status().equals(AppConstants.operatorApprove)
					...
					if (userRole.equalsIgnoreCase(AppConstantDryGoods.operatorRole)) {
						...
						int version = dailyproductiondetailspleateandwoolrollhistoryf006repository
								.getMaximumVersion(date1, shift, machineName, order_no).map(temp -> temp + 1).orElse(1);
						historyRecord.setVersion(version);
...
//				                     }
				...
				if (userRole.equalsIgnoreCase(AppConstantDryGoods.operatorRole)) {
					...
					return new ResponseEntity<>(
							new ApiResponse(false, "Unauthorized to submit Daily Production Report"),
							HttpStatus.FORBIDDEN);
				}
			}
	...
	public ResponseEntity<?> approveRejectDailyProductionPletaeAndWool(ApproveResponse approvalResponse,
			HttpServletRequest http) {
...
//					                    dailyproductionReport.setHod_signature_image(signature);
						...
						String date1 = dailyproductionReport.getDate();
						String shift = dailyproductionReport.getShift();
						String machineName = dailyproductionReport.getMachine_name();
						String order_no = dailyproductionReport.getOrder_no();

						// Update history with the approval details
						history = dailyproductiondetailspleateandwoolrollhistoryf006repository
								.fetchLastSubmittedRecord(date1, shift, machineName, order_no);
...
//					    						dailyproductionReport.setSupervisor_signature_image(signature);
			...
			return new ResponseEntity<>(new ApiResponse(false, "Failed to Approve/Reject Production Report " + msg),
					HttpStatus.BAD_REQUEST);
		}
