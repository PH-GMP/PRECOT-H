...
@Repository
public interface DailyProductionDetailsPleateAndWoolRollF006Repository
		extends JpaRepository<DailyProductionDetailsPleateAndWoolRollF006, Long> {
	...
	List<DailyProductionDetailsPleateAndWoolRollF006> printParam(@Param("date") String date,
			@Param("shift") String shift, @Param("machine_name") String machine_name,
			@Param("order_no") String order_no);

	@Query(value = "SELECT * FROM precot.DRYGOODS_DAILY_PRODUCTION_PLEATE_AND_WOOL_ROLL_F006 WHERE DATE=:date AND SHIFT=:shift AND MACHINE_NAME =:machine_name AND ORDER_NO = :order_no", nativeQuery = true)
	DailyProductionDetailsPleateAndWoolRollF006 getdetailsbyParam(@Param("date") String date,
			@Param("shift") String shift, @Param("machine_name") String machine_name,
			@Param("order_no") String order_no);
