...
const QualityControl_f26 = () => {
  ...
  const [baleLov, setBaleLov] = useState([]);
  const [formData, setFormData] = useState({
    ...
    l_remarks: "",
    reason: "",
    // "chemist_status": "CHEMIST_APPROVED",
    // "chemist_saved_on": "2024-09-17T11:45:01.705+0000",
    // "chemist_saved_by": "chemist",
    // "chemist_saved_id": 20032,
    // "chemist_submit_on": "2024-09-17T11:45:28.750+00"00",
    // "chemist_submit_by": "chemist",
    // "chemist_submit_id": 20032,
    // "chemist_sign": "chemist",
    // "qa_exe_status": "QA_EXE_APPROVED",
    // "qa_exe_submit_on": "2024-09-17T11:50:12.315+0000",
    // "qa_exe_submit_by": "sudhakar",
    // "qa_exe_submit_id": 40052,
    // "qa_exe_sign": "sudhakar",
    // "manager_status": "QC_APPROVED",
  ...
  const handleBack = () => {
    navigate("/Precot/QualityControl/F-026/Summary");
  };
  const rejectFlow = () => {
    setRejectModal(true);
  };
  ...
  const prodDate = async (value) => {
    try {
      ...
      if (response.data) {
        setFormData((prevState) => ({
          ...prevState,
          production_date: response.data[0].value,
        }));
      }
    } catch (error) {
      console.error("Error fetching Job Order Options:", error);
  ...
  const runValidations = () => {
    ...
    if (
      !validateField(
        formData.bale_actual,
        135,
        165,
        "Bale Weight actual Should be within 135 to 165"
      )
    )
      return;
    if (
    ...
    if (
      !validateField(
        formData.micronaire_actual,
        2.8,
        null,
        "Micronaire actual Should be greater than or equal to 2.8"
      )
    )
      return;
    if (
      !validateField(
        ...
        10,
        null,
        "UQL Fiber length actual Should be greater than or equal to 10"
      )
    )
      return;
    if (
      !validateField(
        ...
        7.0,
        null,
        "L(n) mm actual Should be greater than or equal to 7.0"
      )
    )
      return;
  ...
  const handleSave = async () => {
    ...
    if (role == "ROLE_CHEMIST") {
      ...
      payload = {
        ...
        revisionNo: "01",
        formatName: "CERTIFICATE OF ANALYSIS FOR ABSORBENT BLEACHED COTTON",
        ...
        bale_no: formData.bale_no || "NA",
        bale_actual: formData.bale_actual,
        bale_remarks: formData.bale_remarks || "NA",
        ...
        micronaire_actual: formData.micronaire_actual,
        micronaire_remarks: formData.micronaire_remarks || "NA",
        uql_actual: formData.uql_actual,
        uql_remarks: formData.uql_remarks || "NA",
        l_actual: formData.l_actual,
        l_remarks: formData.l_remarks || "NA",
      };
      if (formData.id) {
  ...
  const handleSubmit = async () => {
    let apiurl, payload;
    if (role == "ROLE_CHEMIST") {
      const headerValidate = ["raw_material", "invoice_no", "lot_no", "date"];
      ...
      const getName = (key) => {
        switch (key) {
          ...
          case "pathogen_remarks":
            return "Pathogen (cfu/g)";
          case "odor_actual":
          case "odor_remarks":
            return "Odor";
          case "micronaire_actual":
          case "micronaire_remarks":
            return "Micronaire (µg/inch)";
          case "uql_actual":
          case "uql_remarks":
            return "UQL Fiber length (mm)";
          case "l_actual":
          case "l_remarks":
            return "L(n) mm";
        }
  ...
  const handleBlur = (event) => {
    ...
    // --------- Test Type 1 ---------------
    if (
      (Number(formData.bale_actual) < 135 ||
        Number(formData.bale_actual) > 165) &&
      formData.bale_actual !== ""
    ) {
      validation.add("Bale Weight actual Should be within 135 to 165");
      handleClear("bale_actual");
    }
    if (
      (Number(formData.sinking_actual) < 0 ||
        Number(formData.sinking_actual) >= 10) &&
      formData.sinking_actual !== ""
    ) {
      validation.add("Sinking time actual Should be within 0 to 9");
    ...
    // ---------- Test Type 4 --------------
    if (
      Number(formData.micronaire_actual) < 2.8 &&
      formData.micronaire_actual !== ""
    ) {
      validation.add(
        "Micronaire actual Should be greater than or equal to 2.8"
      );
      handleClear("micronaire_actual");
    }
    if (Number(formData.uql_actual) < 10 && formData.uql_actual !== "") {
      validation.add(
        "UQL Fiber length actual Should be greater than or equal to 10"
      );
      handleClear("uql_actual");
    }
    if (Number(formData.l_actual) < 7.0 && formData.l_actual !== "") {
      validation.add("L(n) mm actual Should be greater than or equal to 7.0");
      handleClear("l_actual");
    }
  ...
  return (
    <>
      <BleachingHeader
        formName={"CERTIFICATE OF ANALYSIS FOR ABSORBENT BLEACHED COTTON"}
        formatNo={"PH-QCL01/F-026"}
        unit={"UNIT H"}
        MenuBtn={
          <Button
        ...
        <Row gutter={[8, 8]} align="middle">
          ...
          <Col xs={24} sm={12} md={6}>
            <Space direction="vertical" style={{ width: "100%" }}>
              <Input
                type="text"
                onChange={(e) => handleInput(e.target.value, "raw_material")}
                name="Raw Material"
                value={formData.raw_material}
                addonBefore="Raw Material :"
                style={{ width: "100%", textAlign: "center" }}
                readOnly={status.fieldStatus}
              />
              <Input
                type="text"
                addonBefore="Production Date :"
                value={formatDate(formData.production_date)}
                style={{
                  width: "100%",
          ...
          <div style={{ height: "50vh" }}>
            <table style={{ height: "100%" }}>
              ...
              <tr>
                ...
                <td style={{ textAlign: "center" }}>Bale Weight (Kg)</td>
                <td style={{ textAlign: "center" }}>150.0</td>
                ...
                <td style={{ textAlign: "center" }}>
                  <Input
                    ...
                    onChange={(e) => {
                      handleInput(e.target.value, "bale_actual");
                    }}
                    onKeyDown={(e) => {
                      handleDigit(e, 7, "need");
                    }}
                    onBlur={handleBlur}
                    value={formData.bale_actual}
                    readOnly={status.fieldStatus}
                  ></Input>
                </td>
                <td style={{ textAlign: "center", padding: "0px" }}>
                  <Select
                    options={Lov7}
                    value={formData.bale_remarks}
                    onBlur={handleBlur}
                    style={{ textAlign: "center", width: "300px" }}
                    onInputKeyDown={(e) => {
                      handleSelectText(e, "bale_remarks");
                    }}
                    onChange={(e) => {
                      handleInput(e, "bale_remarks");
                    }}
                    dropdownStyle={{ textAlign: "center" }}
                    showSearch
                    filterOption={false}
              ...
              <tr>
                ...
                <td style={{ textAlign: "center" }}>EP</td>
                <td style={{ textAlign: "center" }}>
                  Any Foam Present must not <br /> cover the entire
                  <br /> surface of the liquid{" "}
                </td>
                <td style={{ textAlign: "center" }}>
                  <Select
                    options={Lov3}
          ...
          <div style={{ height: "50vh" }}>
            <table style={{ height: "100%" }}>
              ...
              <tr>
                ...
                <td style={{ textAlign: "center" }}>
                  {" "}
                  No intense blue fluorescence
                  <br /> . Few isolated fibers passable
                </td>
                <td style={{ textAlign: "center" }}>
                  <Select
                    options={Lov4}
          ...
          <div style={{ height: "50vh" }}>
            <table style={{ height: "100%" }}>
              ...
              <tr>
                ...
                <td style={{ textAlign: "center" }}>Micronaire (µg/inch)</td>
                <td style={{ textAlign: "center" }}>-</td>
                ...
                <td style={{ textAlign: "center" }}>
                  <Input
                    ...
                    onChange={(e) => {
                      handleInput(e.target.value, "micronaire_actual");
                    }}
                    value={formData.micronaire_actual}
                    onKeyDown={(e) => {
                      handleDigit(e, 100, "need");
                    }}
                    readOnly={status.fieldStatus}
                ...
                <td style={{ textAlign: "center", padding: "0px" }}>
                  <Select
                    options={Lov7}
                    style={{ textAlign: "center", width: "300px" }}
                    value={formData.micronaire_remarks}
                    onInputKeyDown={(e) => {
                      handleSelectText(e, "micronaire_remarks");
                    }}
                    onChange={(e) => {
                      handleInput(e, "micronaire_remarks");
                    }}
                    dropdownStyle={{ textAlign: "center" }}
                    showSearch
                    filterOption={false}
              ...
              <tr>
                ...
                <td style={{ textAlign: "center" }}>UQL Fiber length (mm)</td>
                <td style={{ textAlign: "center" }}>-</td>
                <td style={{ textAlign: "center" }}>≥ 10</td>
                <td style={{ textAlign: "center" }}>
                  <Input
                    ...
                    onChange={(e) => {
                      handleInput(e.target.value, "uql_actual");
                    }}
                    value={formData.uql_actual}
                    onKeyDown={(e) => {
                      handleDigit(e, 100, "need");
                    }}
                    readOnly={status.fieldStatus}
                ...
                <td style={{ textAlign: "center", padding: "0px" }}>
                  <Select
                    options={Lov7}
                    style={{ textAlign: "center", width: "300px" }}
                    value={formData.uql_remarks}
                    onInputKeyDown={(e) => {
                      handleSelectText(e, "uql_remarks");
                    }}
                    onChange={(e) => {
                      handleInput(e, "uql_remarks");
                    }}
                    dropdownStyle={{ textAlign: "center" }}
                    showSearch
                    filterOption={false}
              ...
              <tr>
                ...
                <td style={{ textAlign: "center" }}>L(n) mm</td>
                <td style={{ textAlign: "center" }}>-</td>
                <td style={{ textAlign: "center" }}> ≥ 7.0</td>
                <td style={{ textAlign: "center" }}>
                  <Input
                    type="number"
                    style={{
                    ...
                    onChange={(e) => {
                      handleInput(e.target.value, "l_actual");
                    }}
                    value={formData.l_actual}
                    onKeyDown={(e) => {
                      handleDigit(e, 100, "need");
                    }}
                    readOnly={status.fieldStatus}
                ...
                <td style={{ textAlign: "center", padding: "0px" }}>
                  <Select
                    options={Lov7}
                    style={{ textAlign: "center", width: "300px" }}
                    value={formData.l_remarks}
                    onInputKeyDown={(e) => {
                      handleSelectText(e, "l_remarks");
                    }}
                    onChange={(e) => {
                      handleInput(e, "l_remarks");
                    }}
                    dropdownStyle={{ textAlign: "center" }}
                    showSearch
                    filterOption={false}
...
export default QualityControl_f26;
