...
import com.focusr.Precot.mssql.database.model.productDevelopment.GsmProdDevF001;
...
public ResponseEntity<?> uploadImages(
        String pdsNo,
        MultipartFile innerFilmI,
        MultipartFile outerFilmII,
        MultipartFile innerCartonIII,
        MultipartFile outerCartonIV,
    ...
    try {
        ...
        return new ResponseEntity<>(new ApiResponse(false, "Error processing image files: " + e.getMessage()), HttpStatus.BAD_REQUEST);
    }
}
public ResponseEntity<?> editProductDevelopmentImages(
        String pdsNo,
        MultipartFile innerFilmI,
        MultipartFile outerFilmII,
        MultipartFile innerCartonIII,
...
public ResponseEntity<?> approveReject(ApproveResponse approvalResponse, HttpServletRequest http) {
    ...
    try {
		...
		if(supervisorStatus.equalsIgnoreCase(AppConstantsproductdevelopment.supervisor_Submit) && qcStatus.equalsIgnoreCase(AppConstantsproductdevelopment.waitingStatus)) {
		...
		else if(qcStatus.equalsIgnoreCase(AppConstantsproductdevelopment.QC_APPROVED) && qaStatus.equalsIgnoreCase(AppConstantsproductdevelopment.waitingStatus)) {
            ...
            String departmentId = userRepository.getDepartmentById(userId); // Assuming this method exists
            ...
            if (userRole.equalsIgnoreCase("ROLE_HOD") && approvalResponse.getStatus().equals("Approve")) {
                switch (departmentId) {
                    ...
                    case "7": // PPC
                        ...
                        break;
                    default:
                        return new ResponseEntity<>(new ApiResponse(false, "Invalid Department ID"), HttpStatus.BAD_REQUEST);
                }
...
//                return new ResponseEntity<>(new ApiResponse(true, "Department Approved Successfully"), HttpStatus.OK);
                String departmentApproved = getDepartmentName(departmentId); // Fetch the department name based on the department ID
                return new ResponseEntity<>(new ApiResponse(true, departmentApproved + " Department HOD Approved Successfully"), HttpStatus.OK);

            } else {
                return new ResponseEntity<>(new ApiResponse(false, "Invalid Approval Status or User Role"), HttpStatus.BAD_REQUEST);
            }
        ...
        return new ResponseEntity<>(new ApiResponse(false, "Failed to approve/reject  Product Development Sheet Record: " + msg), HttpStatus.BAD_REQUEST);
    }
...
public ResponseEntity<?> getProductDevelopment(String pdsNo) {
    ...
    try {
        ...
        return new ResponseEntity<>(new ApiResponse(false, "Unable to get Product Development Sheet List: " + msg), HttpStatus.BAD_REQUEST);
    }
	...
	try {
		...
		// F003
		 ...
		 if (AppConstantsproductdevelopment.departmentName.equals(department)
		    ...
		    List<ProductDevelopmentSheetHistoryF001> summaryF01 = productHistoryRepo.findByParams001(
		            fromDate,
		            toDate,
		            PdsNO.isEmpty() ? null : PdsNO // Pass null if pdsNo is empty
		    );
		    ...
		    if (summaryF01 != null && !summaryF01.isEmpty()) {
		        ...
		        return ResponseEntity.ok()
		                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=ProductDevelopmentSheetF001.xlsx")
    ...
    try {
        ...
        if (lastnomenclature == null) {
            return ResponseEntity.ok("Nomenclature - \"YY indicates last 2 digits of year of manufacture;"
            		...
            		+ " 560 is last 3 digit of vendor code(141560); "
            		+ "MM indicate month of mfg. in 2 digits.");
        }
