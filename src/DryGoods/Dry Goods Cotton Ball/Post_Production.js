...
const Post_Production = (props) => {
  ...
  useEffect(() => {
    axios
      .get(
        `${API.prodUrl}/Precot/api/cottonBall/12.GetPostProductionReview?batch_no=${props.batchNo}`,
        {
          headers: {
  ...
  const sendPostProd = () => {
    ...
    if (role == "ROLE_SUPERVISOR") {
      if (postprod.supervisor_date == "") {
        message.warning("All Fields Are Required");
        return;
      }
    }
    if (role == "ROLE_HOD" || role == "ROLE_DESIGNEE") {
      if (postprod.hod_date == "") {
        message.warning("All Fields Are Required");
        return;
      }
    }
    ...
    axios
      .post(
        `${API.prodUrl}/Precot/api/cottonBall/12.SubmitPostProductionReview`,
