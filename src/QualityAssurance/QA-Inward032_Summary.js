...
`;
  ...
  const { formNo, formatName } = state || {};
  console.log("format No by choosen screen", formNo, formatName)
  ...
  const formattedQAinspector = () => {
    if (printResponseData?.[0]?.qa_inspector_submitted_on) {
      const date = moment(printResponseData?.[0]?.qa_inspector_submitted_on);
      if (date.isValid()) {
  ...
  const formattedDate = Array.isArray(printResponseData) && printResponseData.length > 0 && printResponseData[0]?.date
  ...
  const formattedgrDate = Array.isArray(printResponseData) && printResponseData.length > 0 && printResponseData[0]?.gr_date
  ...
  const formattedQaManagerDateTime = printResponseData?.[0]?.qa_manager_submitted_on
    ? formatDateTime(printResponseData?.[0]?.qa_manager_submitted_on)
    : '';

  const formattedQaInspectorDateTime = printResponseData?.[0]?.qa_inspector_submitted_on
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json(); // Parse the response to get error message
  ...
  const fetchGet = async (date) => {
    ...
    try {
      ...
      if (response.data && response.data.suppliers && response.data.suppliers.length > 0) {
  ...
  const fetchPrintGet = async (date) => {
    ...
    try {
      ...
      if (response.data && response.data.body && response.data.body.suppliers && response.data.body.suppliers.length > 0) {
  ...
  const handlePrintSupplier = async (SelectedPrintSupplier) => {
    try {
      ...
      if (response.data && response.data.body.invoices) {
        ...
        console.log("Invoices PPPP", response.data.body.invoices)
      } else {
        console.error("grReceipts not found in the response");
      }
  ...
  const handlePrintInvoiceSelection = async (SelectedInvoicePrint) => {
    try {
      ...
      if (response.data && response.data.body.iirNumbers) {
        ...
        console.log("iirNumbers", response.data.iirNumbers)
      } else {
        console.error("grReceipts not found in the response");
      }
  ...
  const handlePrintiirselection = async (SelectediirNumbers) => {
    try {
      ...
      if (response.data && response.data.body.inspectionDetails) {
        ...
        console.log("PrintData", response.data.body.inspectionDetails)
      } else {
        console.error("grReceipts not found in the response");
      }
  ...
  const handleInvoiceSelection = async (selectedinvoice) => {
    try {
      ...
      if (response.data && response.data.materialDescriptions) {
        ...
        console.log("Invoices", response.data.materialDescriptions)
      } else {
        console.error("grReceipts not found in the response");
      }
  ...
  return (
    <div>
      ...
      <div id="section-to-print">
        ...
        <table style={{ tableLayout: 'fixed' }}>
          <thead style={{ marginTop: '18px' }}>
            <tr>
              <td colSpan={20} style={{ border: "none", height: '35px' }}></td>
            </tr>
    ...
    <td colSpan={20} style={{border: "none" }}></td>
            ...
            <tr>
              <td colSpan={12}>Item Description.:{printResponseData?.[0]?.item_description} </td>
              <td colSpan={8}>AQL Sample Size. :{printResponseData?.[0]?.aql_sample_size} </td>

            </tr>
            <tr>
              <td colSpan={6}>Supplier Name.:{printResponseData?.[0]?.supplier_name} </td>
              <td colSpan={3}>GR Date:{formattedgrDate} </td>
              <td colSpan={3}>GR No:{printResponseData?.[0]?.gr_no}  </td>

              <td colSpan={8}>Item Code:{printResponseData?.[0]?.item_code} </td>

            </tr>

            <tr>
              <td colSpan={4}>Invoice No.:{printResponseData?.[0]?.invoice_no}</td>
              <td colSpan={8}>PO No.: {printResponseData?.[0]?.po_no} </td>
            ...
            <tr>
              <td colSpan={2} style={{ textAlign: 'center' }}>Sr. No.</td>
              <td colSpan={6} style={{ textAlign: 'center' }}>Parameter</td>
              <td colSpan={6} style={{ textAlign: 'center' }}>Specification</td>
              <td colSpan={6} style={{ textAlign: 'center' }}>Observation</td>
            </tr>

            <tr>
              <td colSpan={2}>1</td>
              <td colSpan={6}>Quantity per pack</td>
              <td colSpan={6}>{printResponseData?.[0]?.line1[0].specification}</td>
              <td colSpan={6}>{printResponseData?.[0]?.line1[0].observation}</td>
            </tr>

            <tr>
              <td colSpan={2}>2</td>
              <td colSpan={6}>Total Length of Stick (mm)</td>
              <td colSpan={6}>{printResponseData?.[0]?.line1[1].specification}</td>
              <td colSpan={6}>{printResponseData?.[0]?.line1[1].observation}</td>
            </tr>
            <tr>
              <td colSpan={2}>3</td>
              <td colSpan={6}>Diameter of Stick / Stick Dia</td>
              <td colSpan={6}>{printResponseData?.[0]?.line1[2].specification}</td>
              <td colSpan={6}>{printResponseData?.[0]?.line1[2].observation}</td>
            </tr>
            <tr>
              <td colSpan={2}>4</td>
              <td colSpan={6}>Stick Length (mm)</td>
              <td colSpan={6}>{printResponseData?.[0]?.line1[3].specification}</td>
              <td colSpan={6}>{printResponseData?.[0]?.line1[3].observation}</td>
            </tr>
            <tr>
              <td colSpan={2}>5</td>
              <td colSpan={6}>Weight of 10 buds (gm)</td>
              <td colSpan={6}>{printResponseData?.[0]?.line1[4].specification}</td>
              <td colSpan={6}>{printResponseData?.[0]?.line1[4].observation}</td>
            </tr>
            ...
            <tr>
              <td colSpan={1} style={{ textAlign: 'center' }}>Sr.No</td>
              <td colSpan={2} style={{ textAlign: 'center' }}>Category</td>
              <td colSpan={5} style={{ textAlign: 'center' }}>Defects</td>
              <td colSpan={4} style={{ textAlign: 'center' }}>No. Of Defects Observed</td>
              <td colSpan={4} style={{ textAlign: 'center' }}>Total No. ofDefects Observed</td>
              <td colSpan={4} style={{ textAlign: 'center' }}>Maximum No. of Defects Allowed</td>
            </tr>

            <tr>
              <td colSpan={1}>1</td>
              <td colSpan={2} rowSpan={7}> Critical<br />
                (AQL - Nil)
              </td>
              <td colSpan={5}>Stick Splintering</td>
              <td colSpan={4}>{printResponseData?.[0]?.line2[0].no_of_defects}</td>
              <td colSpan={4} rowSpan={7}>{printResponseData?.[0]?.line2[0].total_no_of_defects}</td>
              <td colSpan={4} rowSpan={7}>{printResponseData?.[0]?.line2[0].maximum_no_of_defects}</td>
            </tr>

            <tr>
              <td colSpan={1}>2</td>
              <td colSpan={5}>Buds breakage at serration</td>
              <td colSpan={4}>{printResponseData?.[0]?.line2[1].no_of_defects}</td>
            </tr>

            <tr>
              <td colSpan={1}>3</td>
              <td colSpan={5}>Inconsistent Dimension </td>
              <td colSpan={4}>{printResponseData?.[0]?.line2[2].no_of_defects}</td>
            </tr>

            <tr>
              <td colSpan={1}>4</td>
              <td colSpan={5}>Inconsistent Length</td>
              <td colSpan={4}>{printResponseData?.[0]?.line2[3].no_of_defects}</td>
            </tr>

            <tr>
              <td colSpan={1}>5</td>
              <td colSpan={5}>Inconsistent Diameter</td>
              <td colSpan={4}>{printResponseData?.[0]?.line2[4].no_of_defects}</td>
            </tr>
            <tr>
              <td colSpan={1}>6</td>
              <td colSpan={5}>Contaminations (Foreign Matl.)</td>
              <td colSpan={4}>{printResponseData?.[0]?.line2[5].no_of_defects}</td>
            </tr>
            <tr>
              <td colSpan={1}>7</td>
              <td colSpan={5}>Weight</td>
              <td colSpan={4}>{printResponseData?.[0]?.line2[6].no_of_defects}</td>
            </tr>

            <tr>
              <td colSpan={1}>1</td>
              <td colSpan={2} rowSpan={4}>Major<br />(AQL - 2.5)</td>
              <td colSpan={5}>No sharp at end</td>
              <td colSpan={4}>{printResponseData?.[0]?.line2[7].no_of_defects}</td>
              <td colSpan={4} rowSpan={4}>{printResponseData?.[0]?.line2[7].total_no_of_defects}</td>
              <td colSpan={4} rowSpan={4}>{printResponseData?.[0]?.line2[7].maximum_no_of_defects}</td>
            </tr>
            <tr>
              <td colSpan={1}>2</td>
              <td colSpan={5}>Less Adhesion Strength</td>
              <td colSpan={4}>{printResponseData?.[0]?.line2[8].no_of_defects}</td>
            </tr>
            <tr>
              <td colSpan={1}>3</td>
              <td colSpan={5}>Uneven Tip</td>
              <td colSpan={4}>{printResponseData?.[0]?.line2[9].no_of_defects}</td>
            </tr>
            <tr>
              <td colSpan={1}>4</td>
              <td colSpan={5}>Moisture Content</td>
              <td colSpan={4}>{printResponseData?.[0]?.line2[10].no_of_defects}</td>
            </tr>
            ...
            <tr>
              <td colSpan={1}>1</td>
              <td colSpan={2} rowSpan={3}>Minor<br />(AQL - 4)</td>
              <td colSpan={5}>Discoloration</td>
              <td colSpan={4}>{printResponseData?.[0]?.line2[11].no_of_defects}</td>
              <td colSpan={4} rowSpan={3}>{printResponseData?.[0]?.line2[11].total_no_of_defects}</td>
              <td colSpan={4} rowSpan={3}> {printResponseData?.[0]?.line2[11].maximum_no_of_defects}</td>
            </tr>

            <tr>
              <td colSpan={1}>2</td>
              <td colSpan={5}>Minor Scratches</td>
              <td colSpan={4}>{printResponseData?.[0]?.line2[12].no_of_defects}</td>
            </tr>

            <tr>
              <td colSpan={1}>3</td>
              <td colSpan={5}>Odors</td>
              <td colSpan={4}>{printResponseData?.[0]?.line2[13].no_of_defects}</td>
            </tr>


            <tr>
              <td colSpan={8} rowspan={2} >Supplier’s COA Reference No.:{printResponseData?.[0]?.coa_reference_no}</td>
            ...
            <tr>
              <td colSpan={20}>Lot Status:{printResponseData?.[0]?.lot_status}</td>

            </tr>

            <tr>
              <td colSpan={20}>Remarks/Reason for Rejection, if any:{printResponseData?.[0]?.remarks} </td>

            </tr>

            <tr>
              <td colSpan={10} style={{ textAlign: 'center' }}>Inspected by:Signature & Date</td>
              <td colSpan={10} style={{ textAlign: 'center' }}>Approved by:Signature & Date</td>
      ...
      >
        ...
        >
          ...
          >
            ...
            {(supplierList && Array.isArray(supplierList) ? supplierList : []).map((Supplier) => (
          ...
          >
            {(invoiceList && Array.isArray(invoiceList) ? invoiceList : []).map((Invoice) => (
      ...
      >
        <div style={{ display: "flex", alignItems: "center", marginBottom: "16px", width: "100%", marginLeft: '20px' }}>
          <label>Date:</label>
          <Input
            type="date"
            size="small"
        ...
        <div style={{ display: "flex", alignItems: "center", marginBottom: "16px", width: "100%" }}>
          ...
          >
            {(PrintSupplier && Array.isArray(PrintSupplier) ? PrintSupplier : []).map((Supplier) => (
              <Option key={Supplier} value={Supplier}>
                {Supplier}
              </Option>
            ))}
        ...
        <div style={{ display: "flex", alignItems: "center", marginBottom: "16px", width: "100%" }}>
          ...
          <Select
            ...
            style={{ fontWeight: "bold", marginRight: "70px", width: "60%" }}
          >
            {(InvoicePrint && Array.isArray(InvoicePrint) ? InvoicePrint : []).map((invoice) => (
              <Option key={invoice} value={invoice}>
                {invoice}
              </Option>
            ))}
        ...
        <div style={{ display: "flex", alignItems: "center", marginBottom: "16px", width: "100%" }}>
          ...
          <Select
            ...
            style={{ fontWeight: "bold", marginRight: "70px", width: "60%" }}
          >
            {(iirNumbers && Array.isArray(iirNumbers) ? iirNumbers : []).map((iirNumber) => (
...
export default QA_Inward032_Sum;