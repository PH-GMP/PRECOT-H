...
const Spunlace_f25_edit = (props) => {
  ...
  // console.loglog("edit page ", shift, date41);
  ...
  const fetchData = async (selectedDate, selectedShift) => {
    ...
    try {
      ...
      // console.loglog("Hell", response.data)
      ...
      if (
        ...
        // console.loglog("shift:", responseData.shift);
        // console.loglog("handSanitizationId:", responseData.handSanitizationId);
      } else {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        // console.loglog("Shift details fetched:", res.data);
      ...
      .catch((err) => {
        // console.loglog("Error fetching shifts:", err);
      });
  }, []);
  ...
  const handleSelectText = (rowIndex, e) => {
    if (
    ...
    if (e.key == "Enter") {
      {
        const updatedIds = idNumbers.map((id, index) =>
          index === rowIndex ? e.target.value : id
        );
        setIdNumbers(updatedIds);
      }
    }
                  ...
                  <tr key={rowIndex}>
                    ...
                    >
                      <Select
                        ...
                        options={idLov}
                        onChange={(e) => handleIdChange(rowIndex, e)}
                        onInputKeyDown={(e) => {
                          handleSelectText(rowIndex, e);
                        }}
  ...
  const validateForm = () => {
    // console.loglog('ID Numbers:', idNumbers);
    // console.loglog('Hour Selections:', hourSelections);
    ...
    for (let i = 0; i < idNumbers.length; i++) {
      ...
      for (let j = 0; j < 8; j++) {
        // console.loglog(`Hour selection for row ${i + 1}, ${getHourLabel(j)}:`, hourSelections[i][j]);
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      formatName: "Spunlace Hand Sanitization Report",
      revisionNo: "01",
      sopNumber: "PH-HRD01-D-0",
  ...
  const handleSubmit = async () => {
    ...
    // console.loglog("Payload:", JSON.stringify(payload, null, 2));
  ...
  const formatName = "Spunlace Hand Sanitization Report";
  const unit = "UNIT-H";
  const role = localStorage.getItem("role");
...
export default Spunlace_f25_edit;
