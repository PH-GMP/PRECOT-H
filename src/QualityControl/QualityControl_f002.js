...
const QCLF002 = () => {
  ...
  const handleSave = () => {
    ...
    const payload = {
      id: mainId || "",
      formatNo: "PH-QCL01/F-002",
      revisionNo: "01",
      formatName: "Microbiology Lab Sample Inward Book",
      refSopNo: "PH-QCL01-D-05",
      dateF002: selectedDate || entryDate,
      month: selectedMonth || uniqueMonth,
      year: selectedYear || uniqueYear,
    ...
    axios
      ...
      .then((response) => {
        message.success(
          "Microbiology Lab Sample Inward Book Saved Successfully.."
        );
  ...
  useEffect(() => {
    console.log("dates", selectedDate, entryDate);
    const fetchData = (date) => {
      const token = localStorage.getItem("token");

      axios
        .get(
          `${    API.prodUrl}/Precot/api/qc/SampleInwardBookF1F2F3/GetByDateF002?dateF002=${date}`,
          {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${token}`,
        ...
        .then((response) => {
          console.log("GET API Response:", response.data[0]);
          ...
          if (response.data && response.data.length > 0) {
            ...
            const username = response.data[0]?.microbiologist_sign;
            ...
            // If there are details, use the first one to set form values
            setFormValues(
              data.details.map((detail) => ({
                id: detail.id,
                date: formatDate(data.dateF002),
                month: data.month,
                year: data.year,
                shift: detail.shift,
  ...
  const handleSubmit = () => {
    ...
    axios
      ...
      .then((response) => {
        message.success(
          "Microbiology Lab Sample Inward Book Submitted Successfully.."
        );
  ...
  const canDisplayButtons = () => {
    ...
    if (roleauth === "ROLE_MICROBIOLOGIST") {
      ...
      if (selectedRow?.microbiologist_status === "MICROBIOLOGIST_APPROVED") {
  ...
  const items = [
    {
      ...
      label: (
        <p>
          <b>Microbiology Inward Details</b>
        </p>
      ),
      children: (
        <div>
