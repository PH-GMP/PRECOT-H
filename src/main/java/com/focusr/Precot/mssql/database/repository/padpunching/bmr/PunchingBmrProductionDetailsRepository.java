...
public interface PunchingBmrProductionDetailsRepository extends JpaRepository<PunchingBmrProductionDetails, Long> {
	...
	@Query(value = "SELECT DISTINCT tf.batchNo FROM tblFPPack tf " + "WHERE tf.POrder IN ("
	...
	@Query(value = "SELECT DISTINCT tf.POrder FROM tblFPPack tf " + "WHERE tf.POrder IN ("
			+ "    SELECT toi.POrder FROM tblOrderInfo toi "
			+ "    INNER JOIN tblProduct tp ON toi.Material = tp.Product "
			+ "    WHERE (:product IS NULL OR tp.Product = :product)" + ") "
			+ "AND tf.PackDt = :manDate", nativeQuery = true)
	...
	@Query(value = "SELECT \r\n" + "    rc.BaleNo AS baleNumber, \r\n" + "    rc.ConsDt AS prodDate, \r\n"
			...
			+ "    rc.POrder =:order AND PackYear = YEAR(GETDATE())" + "", nativeQuery = true)
	...
	@Query(value = "SELECT DISTINCT O.SaleOrder AS saleOrder, \r\n" + "    O.SOitem AS item,  \r\n"
			...
			+ "    TblFPPack FP ON O.POrder = FP.POrder " + "WHERE \r\n" + "    O.sts = 1 \r\n"
			+ "    AND P.Cat = 'Pads'\r\n" + "    AND O.SaleOrder=:saleOrder" + "	   AND O.SOitem =:item\r\n"
			+ "    AND P.Product =:product\r\n" + "GROUP BY \r\n" + "    O.SaleOrder, \r\n" + "    O.SOitem,  \r\n"
			+ "    O.POrder,\r\n" + "    O.Qty,\r\n" + "    P.Bags, \r\n" + "    O.PONo,\r\n" + "    O.Material, \r\n"
			+ "	   O.CustomerPO, \r\n" + "    P.Product, \r\n" + "    P.ProdDesc, \r\n" + "    P.edge, \r\n"
			+ "    O.Pattern, \r\n" + "    O.gsm, \r\n" + "    P.bwgt,\r\n" + "	   FP.batchno, \r\n"
			+ "    fp.PackDt;", nativeQuery = true)
	List<Map<String, Object>> getProductionResponseBySaleOrderItem(@Param("saleOrder") String saleOrder,
			@Param("item") String item, @Param("product") String product);

	// FETCH MAN DATE

	@Query(value = "SELECT " + "    O.SaleOrder AS saleOrder, " + "    O.SOitem AS item,  "
			+ "    O.POrder AS orderNo, " + "    O.Qty AS quantity, " + "    P.Bags AS totalBags, "
			+ "    O.PONo AS poNumber, " + "    O.Material AS material, " + "    P.Product AS product, "
			+ "    P.Cat AS category, " + "    FP.MCN AS machine, "
			+ "    DATEDIFF(day, DATEFROMPARTS(YEAR(FP.PackDt), 1, 1), FP.PackDt) AS julianDay, "
			+ "    FP.ShiftID AS shift, " + "    SUM(FP.NBAG) AS totalPackedBags, "
			+ "    SUM(FP.NBAG * P.bwgt) AS weight " + "FROM " + "    tblOrderInfo O " + "JOIN "
			+ "    tblProduct P ON O.Material = P.Product " + "LEFT JOIN " + "    TblFPPack FP ON O.POrder = FP.POrder "
			+ "WHERE " + "    O.sts = 1 " + "    AND P.Cat = 'Pads' " + "    AND FP.PackDt = :manDate "
			+ "    AND P.Product = :product " + "GROUP BY " + "    O.SaleOrder, " + "    O.SOitem, " + "    O.POrder, "
			+ "    O.Qty, " + "    P.Bags, " + "    O.PONo, " + "    O.Material, " + "    FP.MCN, " + "    P.Product, "
			+ "    FP.ShiftID, " + "  FP.PackDt," + "    P.Cat;", nativeQuery = true)
	List<Map<String, Object>> getProductionResponsebymANDAte(@Param("manDate") String manDate,
	...
	@Query(value = "SELECT " + "    O.SaleOrder AS saleOrder, " + "    O.SOitem AS item,  "
			...
			+ "    tblProduct P ON O.Material = P.Product " + "LEFT JOIN " + "    TblFPPack FP ON O.POrder = FP.POrder "
			+ "WHERE " + "    O.sts = 1 " + "    AND P.Cat = 'Pads' " + "    AND FP.PackDt = :manDate "
			+ "    AND FP.ShiftId =:shiftId " + "    AND P.Product = :product " + "GROUP BY " + "    O.SaleOrder, "
			+ "    O.SOitem, " + "    O.POrder, " + "    O.Qty, " + "    P.Bags, " + "    O.PONo, " + "    O.Material, "
			+ "    FP.MCN, " + "    P.Product, " + "    FP.ShiftID, " + "  FP.PackDt,"
			+ "    P.Cat;", nativeQuery = true)
	List<Map<String, Object>> getProductionResponsebyJulian(@Param("manDate") String manDate,
			@Param("shiftId") Long shiftId, @Param("product") String product);
	...
	@Query(value = "SELECT DISTINCT tci.Bas_Mat \r\n" + "FROM tblOrderInfo toi\r\n"
			+ "INNER JOIN tblProduct tp ON toi.Material = tp.Product\r\n"
	...
	@Query(value = "SELECT \r\n" + "    O.SaleOrder AS saleOrder, \r\n" + "    O.SOitem AS item,  \r\n"
			...
			+ "    O.POrder,\r\n" + "    O.Qty,\r\n" + "    P.Bags, \r\n" + "    O.PONo,\r\n" + "    O.Material, \r\n"
			+ "	   FP.MCN, \r\n" + "    P.Product,\r\n" + "    P.Cat;", nativeQuery = true)
	List<Map<String, Object>> getProductionResponseBySaleOrderItemDryGoods(@Param("saleOrder") String saleOrder,
			@Param("item") String item, @Param("product") String product);
	...
	@Query(value = "SELECT \r\n" + "    O.SaleOrder AS saleOrder, \r\n" + "    O.SOitem AS item,  \r\n"
			...
			+ "    P.Bags, \r\n" + "    O.PONo,\r\n" + "    O.Material, \r\n" + "	   FP.MCN, \r\n"
			+ "    P.Product,\r\n" + "    P.Cat;", nativeQuery = true)
	List<Map<String, Object>> getProductionResponseByManDateDryGoods(@Param("manDate") String manDate,
			@Param("product") String product);
	...
	@Query(value = "SELECT \r\n" + "    O.SaleOrder AS saleOrder, \r\n" + "    O.SOitem AS item,  \r\n"
			...
			+ "    O.SOitem,  \r\n" + "    O.POrder,\r\n" + "    O.Qty,\r\n" + "    P.Bags, \r\n" + "    O.PONo,\r\n"
			+ "    O.Material, \r\n" + "	   FP.MCN, \r\n" + "    P.Product,\r\n" + "    P.Cat;", nativeQuery = true)
	List<Map<String, Object>> getProductionResponseByJulianDryGoods(@Param("manDate") String manDate,
			@Param("shiftId") Long shiftId, @Param("product") String product);
