    ...
    axios
      ...
      .then((response) => {
        if (Array.isArray(response.data) && response.data.length > 0) {
          ...
          const fetchSignatures = fetchedData.map((data) =>
            Promise.all([
              ...
              data.operator_sign
                ? axios
                  ...
                  .then((res) => {
                    ...
                    data.operatorImage = `data:image/jpeg;base64,${base64}`;
                  })
                  .catch((err) => {
                    console.error("Error fetching operator image:", err);
                    data.operatorImage = null; // Handle errors gracefully
                  })
                : Promise.resolve(),
              ...
              data.store_in_charge_sign
                ? axios
                  ...
                  .then((res) => {
                    ...
                    data.storeInChargeImage = `data:image/jpeg;base64,${base64}`;
                  })
                  .catch((err) => {
                    console.error("Error fetching in-charge image:", err);
                    data.storeInChargeImage = null; // Handle errors gracefully
                  })
                : Promise.resolve(),
            ])
          );
    ...
    <div>
      ...
      <div id="section-to-print">
        {Object.entries(
          ...
          // Split the data into chunks of recordsPerPage
          const paginatedData = [];
                ...
                <tbody>
                  ...
                  {pageData.map((data, index) => (
                    <tr key={index}>
                      ...
                      <td colSpan="1" style={{ textAlign: "center" }}>
                        {data.operatorImage && <img className="signature" src={data.operatorImage} alt="Operator Signature" />}
                        <br />
                        {data.operator_sign} <br />
                        {formatDate(data.operator_submit_on)}
                      </td>
                      <td colSpan="1" style={{ textAlign: "center" }}>
                        {data.storeInChargeImage && <img className="signature" src={data.storeInChargeImage} alt="Store In-Charge Signature" />}
                        <br />
