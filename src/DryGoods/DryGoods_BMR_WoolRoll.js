...
import Equipment_Used_Process from "./Dry Goods BMR Wool Roll/Equipment_Used_Process";
import List_Of_Enclosure from "./Dry Goods BMR Wool Roll/List_Of_Enclosure";
import Manufacturing_Steps from "./Dry Goods BMR Wool Roll/Manufacturing_Steps";
import Packing_Material_Issue from "./Dry Goods BMR Wool Roll/Packing_Material_Issue";
import Post_Production from "./Dry Goods BMR Wool Roll/Post_Production";
import Process_Delay from "./Dry Goods BMR Wool Roll/Process_Delay";
import Process_Deviation from "./Dry Goods BMR Wool Roll/Process_Deviation";
import Product_Reconciliation from "./Dry Goods BMR Wool Roll/Product_Reconciliation";
import Product_Release from "./Dry Goods BMR Wool Roll/Product_Release";
import Production_Details from "./Dry Goods BMR Wool Roll/Production_Details";
import Qa_Release from "./Dry Goods BMR Wool Roll/Qa_Release";
import Reference_Documents from "./Dry Goods BMR Wool Roll/Reference_Documents";
import Rework from "./Dry Goods BMR Wool Roll/Rework";
import Verification_Of_Records from "./Dry Goods BMR Wool Roll/Verification_Of_Records";

const DryGoods_BMR_WoolRoll = () => {
  ...
  const [printData, setPrintData] = useState({
    ...
    bmr07manufacturingstepscottonwoolroll: [],
  ...
  useEffect(() => {
    axios
      .get(`${API.prodUrl}/Precot/api/CottonWoolRall/fetchBatchNo`, {
        headers: {
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API.prodUrl}/Precot/api/CottonWoolRall/GetCottonWoolRollPrint?batch_no=${printParams.batchNo}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handlePrint2 = async () => {
    ...
    try {
      setPrintButtonLoading(true);
      const response = await axios.get(
        `${API.prodUrl}/Precot/api/Drygoods/Service/getsliverDetailsForPrintF01?date=${printParams.date}&shift=${printParams.shift}&machine_name=${printParams.machineNamePrint}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  return (
    <div>
      <Modal
        title="Batch Manufacturing Record - Wool Roll (Print)"
        open={isModalPrint}
      ...
      <BleachingHeader
        formName={"Batch Manufacturing Record - Wool Roll"}
        formatNo={"(BMR)"}
        MenuBtn={
          <Button
            type="primary"
        ...
        <table style={{ width: "90%", tableLayout: "fixed" }}>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td colSpan="2" style={{ textAlign: "center", padding: "0.9em" }}>
                {printData &&
                printData.bmr07manufacturingstepscottonwoolroll?.length > 0
                  ? printData.bmr07manufacturingstepscottonwoolroll[0]
                      ?.rdyNoRdy01 == "Ready"
                    ? "✓"
                    : "NA"
                  : "NA"}
            ...
            <tr>
              ...
              <td colSpan="4" style={{ textAlign: "center" }}>
                ...
                RPM<br></br>
                GSM :{" "}
                {printData.bmr07manufacturingstepscottonwoolroll[0]?.cardGsm ||
                  "NA"}{" "}
                <br />
                Width :{" "}
                {printData.bmr07manufacturingstepscottonwoolroll[0]
              ...
              <td colSpan="3" style={{ textAlign: "center", padding: "0.9em" }}>
                ...
                <br></br>
                {formatDateTime(
                  printData.bmr07manufacturingstepscottonwoolroll[0]?.cardDateQa
                ) || "NA"}
              </td>
            </tr>
            <tr>
              ...
              <td colSpan="8" style={{ textAlign: "center", padding: "0.9em" }}>
                ...
                product parameters in sensor cut length, weight, and as per PDS
                and start the machine Paring each and weighing each bag as per
                stander wool roll weight Check metal detector working and ensure
            ...
            <tr>
              ...
              <td colSpan={4} style={{ textAlign: "center", padding: "0.9em" }}>
                PDS No.
                {printData.bmr07manufacturingstepscottonwoolroll[0]?.pdsNo06 ||
                  "NA"}{" "}
                SAP
              </td>
              <td colspan={3} style={{ textAlign: "center", padding: "0.9em" }}>
              ...
              <td colSpan="3" style={{ textAlign: "center", padding: "0.9em" }}>
                ...
                <br></br>
                {formatDateTime(
                  printData.bmr07manufacturingstepscottonwoolroll[0]?.no06DateQa
                ) || "NA"}
              </td>
            </tr>
            <br />
...
export default DryGoods_BMR_WoolRoll;
