...
//	    List<Object[]> getAggregatedResultsByDate(@Param("date") String date);
	...
	@Query(value = "WITH TimeRange AS (\r\n"
			+ "    SELECT S.POrder, S.FTime AS RunFTime, S.TTime AS RunTTime, S.ShiftID\r\n"
			+ "    FROM [PDE].[dbo].[tblSFNG] S\r\n"
			+ "    WHERE S.PackDt = :date\r\n"
			+ "      AND S.MCN IN ('RPBPRESS','RPBPRES1','RPBPRES2')\r\n"
			+ "      AND S.STYPE = 'RUN'),\r\n"
			+ "FilteredStops AS (\r\n"
			+ "    SELECT \r\n"
			+ "         S.SCause,\r\n"
			...
			+ "	        TR.ShiftID\r\n"
			+ "    FROM TimeRange TR\r\n"
			+ "    INNER JOIN [PDE].[dbo].[tblSFNG] S\r\n"
			+ "        ON S.PackDt = :date\r\n"
			+ "        AND S.MCN IN ('RPBPRESS','RPBPRES1','RPBPRES2')\r\n"
			+ "        AND S.STYPE = 'STOP'\r\n"
			+ "		AND S.ShiftID = TR.ShiftID\r\n"
			+ "        AND TR.RunTTime >=  S.TTime \r\n"
			+ "),JoinedStops AS (\r\n"
			+ "    SELECT \r\n"
			+ "        FS.POrder, \r\n"
			+ "        FS.ShiftID, \r\n"
			...
			+ "        M.SCode\r\n"
			+ "    FROM FilteredStops FS\r\n"
			+ "    LEFT JOIN [PDE].[dbo].[tblSCMst] M ON FS.SCause = M.SCause and FS.MGrp = M.MCGrp\r\n"
			+ "),AggregatedResults AS (\r\n"
			+ "    SELECT\r\n"
			+ "        POrder,\r\n"
			+ "        ShiftID,\r\n"
...
//	List<Object[]> getStoppageDetailsF18(@Param("date") String date);
	...
	// F18 - NEW
	@Query(value = "WITH RunOrders AS (\r\n" + "    SELECT ShiftID, TTime, POrder AS POrder\r\n"
			+ "    FROM [PDE].[dbo].[tblSFng]\r\n" + "    WHERE PackDt = :date\r\n" + "        AND Stype = 'RUN'\r\n"
			+ "        AND POrder IS NOT NULL AND POrder <> ''\r\n" + "        AND MCN = 'SRW'\r\n" + "),\r\n"
			+ "StopData AS (\r\n" + "    SELECT ShiftID, TTime AS StopTTime, SCause, TotHrs\r\n"
			+ "    FROM [PDE].[dbo].[tblSFng]\r\n" + "    WHERE PackDt = :date\r\n" + "        AND Stype = 'STOP'\r\n"
			+ "        AND  MCN = 'SRW'\r\n" + "),\r\n" + "MatchedStoppage AS (\r\n" + "    SELECT \r\n"
			+ "        R.POrder,\r\n" + "        R.ShiftID,\r\n" + "        S.SCause,\r\n" + "        S.TotHrs\r\n"
			+ "    FROM StopData S\r\n" + "    CROSS APPLY (\r\n" + "        SELECT TOP 1 R2.POrder, R2.ShiftID\r\n"
			+ "        FROM RunOrders R2\r\n" + "        WHERE R2.ShiftID = S.ShiftID AND R2.TTime > S.StopTTime\r\n"
			+ "        ORDER BY R2.TTime\r\n" + "    ) R\r\n" + "),\r\n" + "StoppageSummary AS (\r\n"
			+ "    SELECT \r\n" + "        M.POrder,\r\n" + "        M.ShiftID,\r\n"
			+ "        SUM(CASE WHEN C.SCode = 'LC' THEN M.TotHrs ELSE 0 END) AS LC_TotalHours,\r\n"
			+ "        SUM(CASE WHEN C.SCode = 'SCL' THEN M.TotHrs ELSE 0 END) AS SCL_TotalHours,\r\n"
			+ "        SUM(CASE WHEN C.SCode = 'CL' THEN M.TotHrs ELSE 0 END) AS CL_TotalHours,\r\n"
			+ "        SUM(CASE WHEN C.SCode = 'MI' THEN M.TotHrs ELSE 0 END) AS MI_TotalHours,\r\n"
			+ "        SUM(CASE WHEN C.SCode = 'ER' THEN M.TotHrs ELSE 0 END) AS ER_TotalHours,\r\n"
			+ "        SUM(CASE WHEN C.SCode = 'MR' THEN M.TotHrs ELSE 0 END) AS MR_TotalHours,\r\n"
			+ "        SUM(CASE WHEN C.SCode NOT IN ('LC','SCL','CL','MI','ER','MR') THEN M.TotHrs ELSE 0 END) AS Others_TotalHours,\r\n"
			+ "        COUNT(CASE WHEN C.SCode = 'LC' THEN 1 ELSE NULL END) AS LC_Count,\r\n"
			+ "        COUNT(CASE WHEN C.SCode = 'SCL' THEN 1 ELSE NULL END) AS SCL_Count,\r\n"
			+ "        COUNT(CASE WHEN C.SCode = 'CL' THEN 1 ELSE NULL END) AS CL_Count,\r\n"
			+ "        COUNT(CASE WHEN C.SCode = 'MI' THEN 1 ELSE NULL END) AS MI_Count,\r\n"
			+ "        COUNT(CASE WHEN C.SCode = 'ER' THEN 1 ELSE NULL END) AS ER_Count,\r\n"
			+ "        COUNT(CASE WHEN C.SCode = 'MR' THEN 1 ELSE NULL END) AS MR_Count,\r\n"
			+ "        COUNT(CASE WHEN C.SCode NOT IN ('LC','SCL','CL','MI','ER','MR') THEN 1 ELSE NULL END) AS Others_Count\r\n"
			+ "    FROM MatchedStoppage M\r\n" + "    JOIN [PDE].[dbo].[tblSCMst] C ON M.SCause = C.SCause\r\n"
			+ "    GROUP BY M.POrder, M.ShiftID\r\n" + "),\r\n" + "RNwtSummary AS (\r\n" + "    SELECT \r\n"
			+ "        R.POrder,\r\n" + "        R.ShiftID,\r\n" + "        ISNULL(SUM(G.RNwt), 0) AS TotalRNwt,\r\n"
			+ "        MAX(G.Brand) AS Brand\r\n" + "    FROM RunOrders R\r\n"
			+ "    LEFT JOIN [PDE].[dbo].[tblRGoods] G\r\n"
			+ "        ON R.POrder = G.POrder AND R.ShiftID = G.ShiftID AND G.PackDt = :date AND G.isApproved <> 'R'\r\n"
			+ "    GROUP BY R.POrder, R.ShiftID\r\n" + ")\r\n" + "SELECT \r\n" + "    R.POrder,\r\n"
			+ "    R.TotalRNwt,\r\n" + "    R.Brand,\r\n" + "    R.ShiftID,\r\n"
			+ "    ISNULL(S.LC_TotalHours, 0) AS LC_TotalHours,\r\n"
			+ "    ISNULL(S.SCL_TotalHours, 0) AS SCL_TotalHours,\r\n"
			+ "    ISNULL(S.CL_TotalHours, 0) AS CL_TotalHours,\r\n"
			+ "    ISNULL(S.MI_TotalHours, 0) AS MI_TotalHours,\r\n"
			+ "    ISNULL(S.ER_TotalHours, 0) AS ER_TotalHours,\r\n"
			+ "    ISNULL(S.MR_TotalHours, 0) AS MR_TotalHours,\r\n"
			+ "    ISNULL(S.Others_TotalHours, 0) AS Others_TotalHours,\r\n"
			+ "    ISNULL(S.LC_Count, 0) AS LC_Count,\r\n" + "    ISNULL(S.SCL_Count, 0) AS SCL_Count,\r\n"
			+ "    ISNULL(S.CL_Count, 0) AS CL_Count,\r\n" + "    ISNULL(S.MI_Count, 0) AS MI_Count,\r\n"
			+ "    ISNULL(S.ER_Count, 0) AS ER_Count,\r\n" + "    ISNULL(S.MR_Count, 0) AS MR_Count,\r\n"
			+ "    ISNULL(S.Others_Count, 0) AS Others_Count\r\n" + "FROM RNwtSummary R\r\n"
			+ "LEFT JOIN StoppageSummary S ON R.POrder = S.POrder AND R.ShiftID = S.ShiftID\r\n"
			+ "ORDER BY R.ShiftID, R.POrder", nativeQuery = true)
	List<Object[]> getStoppageDetailsF18(@Param("date") String date);
...
//	@Query(value = "WITH RunOrders AS (\r\n" + "\r\n"
//			+ "SELECT ShiftID, TTime, POrder1 AS POrder FROM [PDE].[dbo].[tblSSpl]\r\n" + "\r\n"
//			+ "WHERE PackDt = :date AND Stype = 'RUN' AND POrder1 IS NOT NULL AND POrder1 <> ''\r\n" + "\r\n"
//			+ "UNION\r\n" + "\r\n" + "SELECT ShiftID, TTime, POrder2 FROM [PDE].[dbo].[tblSSpl]\r\n" + "\r\n"
//			+ "WHERE PackDt = :date AND Stype = 'RUN' AND POrder2 IS NOT NULL AND POrder2 <> ''\r\n" + "\r\n"
//			+ "UNION\r\n" + "\r\n" + "SELECT ShiftID, TTime, POrder3 FROM [PDE].[dbo].[tblSSpl]\r\n" + "\r\n"
//			+ "WHERE PackDt = :date AND Stype = 'RUN' AND POrder3 IS NOT NULL AND POrder3 <> ''\r\n" + "\r\n" + "),\r\n"
//			+ "\r\n" + "\r\n" + "\r\n" + "StopData AS (\r\n" + "\r\n"
//			+ "SELECT ShiftID, TTime AS StopTTime, SCause, TotHrs\r\n" + "\r\n" + "FROM [PDE].[dbo].[tblSSpl]\r\n"
//			+ "\r\n" + "WHERE PackDt = :date AND Stype = 'STOP'\r\n" + "\r\n" + "),\r\n" + "\r\n" + "\r\n" + "\r\n"
//			+ "MatchedStoppage AS (\r\n" + "\r\n" + "SELECT\r\n" + "\r\n" + "R.POrder,\r\n" + "\r\n" + "R.ShiftID,\r\n"
//			+ "\r\n" + "S.SCause,\r\n" + "\r\n" + "S.TotHrs\r\n" + "\r\n" + "FROM StopData S\r\n" + "\r\n"
//			+ "CROSS APPLY (\r\n" + "\r\n" + "SELECT TOP 1 R2.POrder, R2.ShiftID\r\n" + "\r\n" + "FROM RunOrders R2\r\n"
//			+ "\r\n" + "WHERE R2.ShiftID = S.ShiftID AND R2.TTime > S.StopTTime\r\n" + "\r\n" + "ORDER BY R2.TTime\r\n"
...
//	List<Object[]> fetchStoppageDetails1(@Param("date") String date);
	...
	@Query(value = "WITH RunOrders AS (\r\n"
			+ "    SELECT ShiftID, TTime, POrder1 AS POrder\r\n"
			+ "    FROM [PDE].[dbo].[tblSSpl]\r\n"
			+ "    WHERE PackDt = :date\r\n"
			+ "        AND Stype = 'RUN'\r\n"
			+ "        AND POrder1 IS NOT NULL AND POrder1 <> ''\r\n"
			+ "        AND (MACHINE IN ('SL', 'SL1') OR MACHINE IS NULL)\r\n"
			+ "    UNION\r\n"
			+ "    SELECT ShiftID, TTime, POrder2\r\n"
			+ "    FROM [PDE].[dbo].[tblSSpl]\r\n"
			+ "    WHERE PackDt = :date\r\n"
			+ "        AND Stype = 'RUN'\r\n"
			+ "        AND POrder2 IS NOT NULL AND POrder2 <> ''\r\n"
			+ "        AND (MACHINE IN ('SL', 'SL1') OR MACHINE IS NULL)\r\n"
			+ "    UNION\r\n"
			+ "    SELECT ShiftID, TTime, POrder3\r\n"
			+ "    FROM [PDE].[dbo].[tblSSpl]\r\n"
			+ "    WHERE PackDt = :date\r\n"
			+ "        AND Stype = 'RUN'\r\n"
			+ "        AND POrder3 IS NOT NULL AND POrder3 <> ''\r\n"
			+ "        AND (MACHINE IN ('SL', 'SL1') OR MACHINE IS NULL)\r\n"
			+ "),\r\n"
			+ "StopData AS (\r\n"
			+ "    SELECT ShiftID, TTime AS StopTTime, SCause, TotHrs\r\n"
