...
const Qa_Release = (props) => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${API.prodUrl}/Precot/api/cottonBall/13.GetQaRelease?batch_no=${props.batchNo}`,
          {
            headers: {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API.prodUrl}/Precot/api/cottonBall/13.SaveQaRelease`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleSubmit = async () => {
    ...
    for (let detail of qaData.details) {
      for (let key in detail) {
        if (
          ...
          message.warning("Some required fields are empty");
          return;
        }
      }
    }
    ...
    try {
      ...
      const response = await axios.post(
        `${API.prodUrl}/Precot/api/cottonBall/13.SubmitQaRelease`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${token}`,
