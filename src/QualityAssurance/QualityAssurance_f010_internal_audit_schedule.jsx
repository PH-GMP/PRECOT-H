...
const QualityAssurance_f010_internal_audit_schedule = () => {
      ...
      try {
        ...
        if (
          ...
          if (DepartmentsResponse.data) {
            const fetchedDepartments = DepartmentsResponse.data.map(
              (Department, index) => ({
  ...
  const columns = [
    {
      title: "Department To Be Audited",
  ...
  return (
    <div>
      <style>
        {`
          span.ant-select-selection-item .ant-btn {
            display: none;
          }
        `}
      </style>
