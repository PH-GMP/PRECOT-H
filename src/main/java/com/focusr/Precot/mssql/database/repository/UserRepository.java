...
// QC VIJAY
	...
	@Query(value = "SELECT u.username AS user_username, u.name AS user_name, r.name AS role_name "
			...
			+ "AND u.is_active = 'Y' " + "AND u.DEPARTMENT_ID = :department_id OR du.dept_id IN (:department_id)", nativeQuery = true)
...
@Query(value ="SELECT DEPARTMENT_ID FROM precot.USER_LOGIN_DETAILS WHERE email = :mail", nativeQuery = true)
	...
	// VIJAY
		...
		@Query(value = "SELECT users.name FROM precot.USER_LOGIN_DETAILS users, "
		        + "precot.ROLES_MAP_BY_USERS user_role_map, "
		        + "precot.USER_ROLES roles "
		        + "WHERE users.id = user_role_map.user_id "
		        + "AND roles.id = user_role_map.role_id "
		         + "AND roles.id != 1 " 
		        + "ORDER BY users.id", nativeQuery = true)
		...
		@Query(value = "SELECT TOP 1 u.email FROM precot.USER_LOGIN_DETAILS u " +
		        "JOIN precot.ROLES_MAP_BY_USERS rm ON u.id = rm.user_id " +
		        "JOIN precot.USER_ROLES r ON rm.role_id = r.id " +
		        "JOIN precot.DEPARTMENT d ON u.DEPARTMENT_ID = d.id " +
		        "WHERE r.name = :roleName AND d.DEPARTMENT = :departmentName AND u.is_active = 'Y'", 
		nativeQuery = true)
		...
		@Query(value = "SELECT u.email FROM precot.USER_LOGIN_DETAILS u " +
		        "JOIN precot.ROLES_MAP_BY_USERS rm ON u.id = rm.user_id " +
		        "JOIN precot.USER_ROLES r ON rm.role_id = r.id " +
		        "WHERE r.name = :roleName  AND u.is_active = 'Y'", 
		nativeQuery = true)
	    ...
	    BleachHodHrQaDetails getContactByRoleAndDepartment(@Param("role") String role, @Param("departmentId") String departmentId);
		...
		@Query(value = "SELECT u.email, u.id, u.username " +
	            ...
	            + "LEFT JOIN precot.DEPARTMENT_MAP_BY_USERS du ON du.user_id = u.id " +
	            "JOIN precot.USER_ROLES r ON rm.role_id = r.id " +
	            "WHERE u.DEPARTMENT_ID = :departmentId  OR du.dept_id IN (:department_id)" +
	            "AND r.name = 'ROLE_HOD' " +
	            "AND u.is_active = 'Y'", 
	    nativeQuery = true)
	...
	List<Object[]> getUsersByRoleName(@Param("roleName") String roleName);
		...
		@Query(value = "SELECT u.username " +
	            ...
	            "JOIN precot.USER_ROLES r ON rm.role_id = r.id "
	            + "LEFT JOIN precot.DEPARTMENT_MAP_BY_USERS du ON du.user_id = u.id " +
	            "WHERE r.name IN ('QA_MANAGER', 'ROLE_QA') " +
	            "AND u.is_active = 'Y' " +
	            "AND u.DEPARTMENT_ID = 12 OR du.dept_id IN (12)",
	            nativeQuery = true)
...
//		Giri PPC Contract Review Meeting 
		...
		@Query(value = "SELECT u.username " +
	            ...
	            "WHERE r.name = 'ROLE_HOD' " +
	            "AND u.is_active = 'Y' " +
	            "AND u.DEPARTMENT_ID IN (1,2,3,4,5,6,9,12) OR du.dept_id IN (1,2,3,4,5,6,9,12)",
	            nativeQuery = true)
