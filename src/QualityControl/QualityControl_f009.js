...
const QualityControlF009 = () => {
  ...
  const [formValues, setFormValues] = useState({
    ...
    month: "",
    year: "",
    standardSolutionDisWater: "",
    standardSolution100NTU: "",
    standardSolution10NTU: "",
    remarks: "",
  });
        ...
        const fetchData = async () => {
          ...
          try {
            const response = await axios.get(
              `${    API.prodUrl}/Precot/api/chemicaltest/CLF009?date=${uniqueDate}&month=${formValues.month}&year=${formValues.year}&eq_no=${uniqueEqNo}`,
              {
                headers: {
            ...
            if (Array.isArray(response.data) && response.data.length > 0) {
              ...
              setMainID(response.data[0].lab_id);
              ...
              if (
                ...
                if (
                  ...
                  setTimeout(() => {
                    navigate("/Precot/QualityControl/F-009/Summary");
                  }, 1500);
                }
              }
              ...
              setFormValues((prevValues) => ({
                ...
                eqno: response.data[0].eq_id_no, // Ensure eqno is set
                month: response.data[0].month || "",
                year: response.data[0].year || "",
                standardSolutionDisWater: response.data[0].dis_water || "",
                standardSolution100NTU: response.data[0].ntu_100 || "",
                standardSolution10NTU: response.data[0].ntu_10 || "",
  ...
  const handleSave = async () => {
    ...
    const payload = {
      lab_id: mainID || "",
      format_no: "PH-QCL01/F-009",
      revision_no: "01",
      ref_sop_no: "PH-QCL01-D-04",
      eq_id_no: formValues.eqno, // Using 'eqno' from formValues
      ...
      dis_water: formValues.standardSolutionDisWater, // Using 'standardSolutionDisWater'
      ntu_100: formValues.standardSolution100NTU, // Using 'standardSolution100NTU'
      ntu_10: formValues.standardSolution10NTU,
    };
    ...
    try {
      ...
      const response = await axios.post(
        `${    API.prodUrl}/Precot/api/chemicaltest/CLF009/save/turbiditycreport
        `,
        payload,
        {
          headers: {
      ...
      message.success("Turbidity Calibration Report saved successfully!");
  ...
  const handleSubmit = async () => {
    ...
    if (
      !formValues.standardSolutionDisWater &&
      !formValues.standardSolution100NTU &&
      !formValues.standardSolution10NTU &&
      !formValues.remarks
    ) {
    ...
    if (!formValues.standardSolutionDisWater) {
      setSubmitLoading(false);
      message.error("Standard Solution Dis.Water is required.");
      return; // Exit the function if validation fails
    }

    if (!formValues.standardSolution100NTU) {
      setSubmitLoading(false);
      message.error("Standard Solution 100NTU is required.");
      return; // Exit the function if validation fails
    }

    if (!formValues.standardSolution10NTU) {
      setSubmitLoading(false);
      message.error("Standard Solution 10NTU is required.");
      return; // Exit the function if validation fails
    }
    ...
    const payload = {
      ...
      month: formValues.month, // Using 'month' from formValues
      year: formValues.year, // Using 'year' from formValues
      remark: formValues.remarks.trim() || "NA", // Using 'remarks' from formValues
      date: formValues.date, // Using 'date' from formValues
      dis_water: formValues.standardSolutionDisWater, // Using 'standardSolutionDisWater'
      ntu_100: formValues.standardSolution100NTU, // Using 'standardSolution100NTU'
      ntu_10: formValues.standardSolution10NTU,
    ...
    try {
      // Make the API call using Axios
      const response = await axios.post(
        `${    API.prodUrl}/Precot/api/chemicaltest/CLF009/submit/turbiditycreport`,
        payload,
        {
          headers: {
            "Content-Type": "application/json",
      ...
      message.success("Turbidity Calibration Report submitted successfully!");
      setSubmitLoading(false);
  ...
  const handleApprove = async () => {
    ...
    const res = await axios
      .post(
        `${    API.prodUrl}/Precot/api/chemicaltest/CLF009/approval`,
        {
          id: mainID,
          status: "Approve",
        },
  ...
  const items = [
    {
      ...
      label: (
        <p>
          <b>Turbidity Calibration Report Details</b>
        </p>
      ),
      children: (
        <div>
              ...
              <tbody>
                <tr>
                  ...
                  <td style={{ textAlign: "center", padding: "10px" }}>
                    <Input
                      type="number"
                      value={formValues.standardSolutionDisWater}
                      onChange={(e) =>
                        handleInputChange(
                          "standardSolutionDisWater",
                          e.target.value
                  ...
                  <td style={{ textAlign: "center", padding: "10px" }}>
                    <Input
                      type="number"
                      value={formValues.standardSolution100NTU}
                      onChange={(e) =>
                        handleInputChange(
                          "standardSolution100NTU",
                          e.target.value
                      ...
                      onBlur={(e) => {
                        ...
                        if (value < 99.5 || value > 100.5) {
                          message.error(
                            "standardSolution100NTU value must be between 99.50 and 100.50"
                          );
                        }
                      }}
                    />
                  ...
                  <td style={{ textAlign: "center", padding: "10px" }}>
                    <Input
                      type="number"
                      value={formValues.standardSolution10NTU}
                      onChange={(e) =>
                        handleInputChange(
                          "standardSolution10NTU",
                          e.target.value
                      ...
                      onBlur={(e) => {
                        const value = parseFloat(e.target.value);
                        if (value < 9.9 || value > 10.1) {
                          message.error(
                            "standardSolution10NTU value must be between 9.90 and 10.10"
                          );
                        }
                      }}
                    />
...
export default QualityControlF009;
