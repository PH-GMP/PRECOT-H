...
const QualityControl_f26A = () => {
      ...
      const fetchData = async () => {
        try {
          const response = await axios.get(
            `${API.prodUrl}/Precot/api/QcForm/getCotttonPadF26A?product=${productName}&customer=${customerName}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
          ...
          if (response.data.length == 0) {
            if (role == "QA_EXECUTIVE") {
              ...
              setTimeout(() => {
                navigate("/Precot/QualityControl/F-026A/Summary");
              }, 1000);
              return;
            }
  ...
  const runValidations = () => {
    ...
    const ppbStandard = Number(formData.ppb_standard);
    const lowerBound1 = ppbStandard;
    const upperBound1 = ppbStandard + 2;

    const grossWeightStandard = Number(formData.gross_weight_standard);
    const grossWeighttenPercent = (grossWeightStandard / 100) * 10;
    const grossWeighttwentyFivePercent = (grossWeightStandard / 100) * 25;
    const grossWeightlowerBound = grossWeightStandard - grossWeighttenPercent;
    const grossWeightupperBound =
      grossWeightStandard + grossWeighttwentyFivePercent;
    ...
    if (
      !validateField(
        formData.no_of_pad_actual,
        lowerBound1,
        upperBound1,
        `No. of pads per bag actual should be within ${lowerBound1} to ${upperBound1}`
      )
    )
      return;
    if (
      !validateField(
        ...
        grossWeightlowerBound,
        grossWeightupperBound,
        `Gross Weight actual should be within ${grossWeightlowerBound} to ${grossWeightupperBound} `
      )
    )
      return;
    if (
  ...
  const handleSave = async () => {
    ...
    if (role == "ROLE_CHEMIST") {
      ...
      apiurl = `${API.prodUrl}/Precot/api/QcForm/SaveCottonPadsF26A`;
      payload = {
        materialDocNo: "",
        formatNo: "PH-QCL01-F-026A",
        revisionNo: "01",
        formatName: "CERTIFICATE OF ANALYSIS FOR COTTON PADS",
        ...
        no_of_pad_actual: formData.no_of_pad_actual,
        ppb_standard: formData.ppb_standard,
        no_of_pad_remarks: formData.no_of_pad_remarks || "NA",
      ...
      apiurl = `${API.prodUrl}/Precot/api/QcForm/ApproveOrRejectF26A`;
    ...
    if (role == "ROLE_CHEMIST") {
      ...
      const getName = (key) => {
        switch (key) {
          ...
          case "gsm_remarks":
            return "GSM";
          case "no_of_pad_actual":
          case "ppb_standard":
            return "No. of pads per bag standard";
          case "no_of_pad_remarks":
            return "No. of pads per bag";
          case "gross_weight_standard":
            return "gross_weight_standard";
          case "gross_weight_actual":
          case "gross_weight_remarks":
      ...
      apiurl = `${API.prodUrl}/Precot/api/QcForm/SubmitCottonPadsF26A`;
      payload = {
        materialDocNo: "",
        formatNo: "PH-QCL01-F-26",
        revisionNo: "",
  ...
  const handleBlur = (event) => {
    ...
    if (
      (Number(formData.no_of_pad_actual) < lowerBound1 ||
        Number(formData.no_of_pad_actual) > upperBound1) &&
      formData.no_of_pad_actual !== ""
    ) {
      validation.add(
        `No. of pads per bag actual should be within ${lowerBound1} to ${upperBound1}`
      );
      handleClear("no_of_pad_actual");
    }
    if (
      (Number(formData.gross_weight_actual) < grossWeightlowerBound ||
        Number(formData.gross_weight_actual) > grossWeightupperBound) &&
      formData.gross_weight_actual !== ""
    ) {
      grossWeightupperBound;
      validation.add(
        `Gross weight of the bag actual should be within ${grossWeightlowerBound} to ${grossWeightupperBound}`
      );
          ...
          <Col xs={24} sm={12} md={6}>
            <Space direction="vertical" style={{ width: "100%" }}>
              ...
              <Input
                ...
                onChange={(e) =>
                  handleInput(e.target.value, "product_dimension")
                }
                readOnly={status.fieldStatus}
              />
            </Space>
              ...
              <tr>
                ...
                <td style={{ textAlign: "center" }}>
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      handleInput(e.target.value, "ppb_standard");
                    }}
                    onKeyDown={(e) => {
                      handleDigit(e, 1000, "need");
                    }}
                    value={formData.ppb_standard}
                    readOnly={status.fieldStatus}
                  ></Input>
                </td>
                ...
                <td style={{ textAlign: "center" }}>
                  <Input
                    ...
                    value={formData.no_of_pad_actual}
                    onChange={(e) => {
                      handleInput(e.target.value, "no_of_pad_actual");
                    }}
                    onKeyDown={(e) => {
                      handleDigit(e, 3, "no need");
                    }}
                    readOnly={status.fieldStatus || formData.ppb_standard == ""}
                    onBlur={handleBlur}
                  ></Input>
                </td>
                <td style={{ textAlign: "center", padding: "0px" }}>
                  <Select
                    options={Lov1}
                    value={formData.no_of_pad_remarks}
                    style={{ textAlign: "center", width: "300px" }}
                    onInputKeyDown={(e) => {
                      handleSelectText(e, "no_of_pad_remarks");
                    }}
                    onChange={(e) => {
                      handleInput(e, "no_of_pad_remarks");
                    }}
                    dropdownStyle={{ textAlign: "center" }}
                    showSearch
                    filterOption={false}
              ...
              <tr>
                ...
                <td style={{ textAlign: "center" }}>
                  <Input
                    ...
                    onKeyDown={(e) => {
                      handleDigit(e, 15, "gross_weight_standard");
                    }}
                    readOnly={status.fieldStatus}
                    onBlur={handleBlur}
                  ></Input>
...
export default QualityControl_f26A;
