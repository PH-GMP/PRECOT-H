...
const Production_Details = (props) => {
      ...
      try {
        ...
        if (response.data.length > 0) {
          ...
          const fetchData = async () => {
            try {
              const response = await axios.get(
                `${API.prodUrl}/Precot/api/cottonPleat/getOrderByBatchNo?batchNo=${batchNo}`,
                {
                  headers: {
                    Authorization: `Bearer ${token}`,
                  },
  ...
  const handlePDE = async (prodBatchNo, prodOrderNo) => {
    if (prodBatchNo !== "" && prodOrderNo !== "") {
      try {
        ...
        const response = await axios.get(
          `${API.prodUrl}/Precot/api/cottonPleat/getProductionDetailsBatchOrder?batchNo=${props.batchNo}&orderNo=${productionDetails.order_no}&fromdate=${productionDetails.start_date}&todate=${productionDetails.end_date}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const SaveProductionDetails = async () => {
    ...
    try {
      const response = await axios.post(
        `${API.prodUrl}/Precot/api/cottonPleat/01.SaveProductionDetails`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      try {
        ...
        const data = {
          ...
          manufactureCompletionDate:
            productionDetails.manufactureCompletionDate,
          manufactureCompletionTime:
            productionDetails.manufactureCompletionTime,
        ...
        const response = await axios.post(
          `${API.prodUrl}/Precot/api/cottonPleat/01.SubmitProductionDetails`,
          newSave ? data2 : data,
          {
            headers: {
  ...
  const getExportRadioValue = () => {
    // if (productionDetails.bmr01rp01productiondetailsSap.length > 0) {
    //   const { IN_HOUSE_EXPORT } =
    //     productionDetails.bmr01rp01productiondetailsSap[0];
    //   if (IN_HOUSE_EXPORT === "Export") {
    //     return "TICK";
    //   } else {
    //     return "NA";
    //   }
        ...
        <>
          ...
          >
            ...
            <tr>
              ...
              <td style={{}}>{productionDetails.boxPackDate}</td>
            </tr>
            <tr>
              <td
                colSpan={1}
