...
const QCLF003 = () => {
  ...
  // Utility function to determine the date
  const determineDate = () => {
  ...
  const handleSave = () => {
    ...
    const payload = {
      id: mainId || "",
      formatNo: "PH-QCL01/F-003",
      revisionNo: "01",
      formatName: "ETP Lab Sample Inward Book",
      refSopNo: "PH-QCL01-D-05",
      dateF003: selectedDate || entryDate,
      month: selectedMonth || uniqueMonth,
      year: selectedYear || uniqueYear,
    ...
    axios
      ...
      .then((response) => {
        message.success("ETP Lab Sample Inward Book Saved Successfully..");
  ...
  useEffect(() => {
    ...
    const fetchData = (date) => {
      ...
      axios
        .get(
          `${    API.prodUrl}/Precot/api/qc/SampleInwardBookF1F2F3/GetByDateF003?dateF003=${date}`,
          {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${token}`,
        ...
        .then((response) => {
          ...
          const data = response.data[0]; // First record from the response

          // Set the main ID for further use
          setMainId(data.id);

          setSelectedRow(response.data[0]);

          const username = response.data[0]?.etp_sign;
          ...
          // Check if details are available, and if so, extract values
          if (data.details && data.details.length > 0) {
            setFormValues(
              data.details.map((detail) => ({
                id: detail.id,
                date: formatDate(data.dateF003),
                month: data.month,
                year: data.year,
                shift: detail.shift,
            ...
            // No details available, reset relevant form fields
            setFormValues([
              {
                id: "",
  ...
  }, [entryDate, selectedDate, selectedMonth, selectedYear]);

  const handleSubmit = () => {
    console.log("selected month", selectedMonth);
    console.log("selected year", selectedYear);
    console.log("selected dates", selectedDate, entryDate);
    console.log("submit year month", selectedMonth, selectedYear);
    ...
    axios
      ...
      .then((response) => {
        message.success("ETP Lab Sample Inward Book Submitted Successfully..");
  ...
  const canEdit = () => {
    if (
      ...
      return !(selectedRow?.etp_status === "ETP_APPROVED");
    } else {
      return false;
    }
  };
  ...
  const canDisplayButtons = () => {
    ...
    if (roleauth === "ROLE_CHEMIST" || roleauth === "ROLE_ETP") {
      console.log("enteredtorolesuccescondtion");
      if (selectedRow?.etp_status === "ETP_APPROVED") {
  ...
  let formattedETPDate;
  if (selectedRow.etp_submit_on) {
    formattedETPDate = moment(selectedRow.etp_submit_on).format(
      "DD/MM/YYYY HH:mm"
    );
  } else {
    formattedETPDate = "";
        ...
        >
          ...
          >
            ...
            <tr style={{ height: "35px" }}>
              ...
              >
                {selectedRow?.etp_status === "ETP_APPROVED" && (
                  <>
                    {getImage1 && (
                      <img
                        className="signature"
                    ...
                    <br />
                    {selectedRow && selectedRow.etp_sign}
                    <br />
                    {formattedETPDate}
                  </>
                )}
  ...
  return (
    <>
      ...
      <BleachingHeader
        ...
        buttonsArray={[
          roleauth === "ROLE_ETP" || roleauth === "ROLE_CHEMIST" ? (
            <>
              <Button
                loading={saveLoading}
                type="primary"
